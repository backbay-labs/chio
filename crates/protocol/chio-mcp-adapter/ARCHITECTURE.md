...
- `url_elicitation.rs` maps wrapped-server `-32042` errors into `KernelError::UrlElicitationsRequired`. It validates each URL-mode operation's `message`, `url`, and `elicitationId`, rejecting empty or padded identifiers and non-HTTP(S) or userinfo-bearing URLs. Form-mode and mixed-mode elicitations are rejected on this wrapped-tool path. If the error arrives after the kernel polls a dispatched invocation, the outcome is unknown: the kernel does not store a pending elicitation, retains admitted state and exposure pending reconciliation, and signs `Decision::Incomplete`.
