...
These fixtures are deterministic conformance captures for the OpenAI Responses API shape supported by `crates/protocol/chio-openai-adapter` as of the pinned snapshot. They are authored from the in-repo adapter and conformance corpus shapes. They do not claim live network capture.
