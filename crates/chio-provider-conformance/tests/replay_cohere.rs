...
use chio_provider_conformance::{replay_cohere_fixture, ReplayMode};
...
fn fixture_paths() -> Vec<PathBuf> {
    ...
    let entries = match fs::read_dir(fixture_dir()) {
        Ok(entries) => entries,
        Err(error) => panic!("read cohere fixture dir: {error}"),
    };
    for entry in entries {
        let entry = match entry {
            Ok(entry) => entry,
            Err(error) => panic!("read cohere fixture entry: {error}"),
        };
        let path = entry.path();
...
#[test]
fn replays_all_cohere_fixtures_with_canonical_byte_assertions() {
    let paths = fixture_paths();
    assert_eq!(
        paths.len(),
        12,
        "Cohere corpus must contain exactly 12 NDJSON fixtures"
    );
    ...
    for path in &paths {
        ...
        assert!(
            body.contains("\"x-cohere-api-version\":\"2025-04\""),
            "{} did not pin x-cohere-api-version",
            path.display()
        );
        assert!(
        ...
        let outcome = match replay_cohere_fixture(path) {
            Ok(outcome) => outcome,
            Err(error) => panic!("Cohere replay failed for {}: {error}", path.display()),
        };
        assert!(matches!(
            outcome.mode,
    ...
    assert_eq!(
        kernel_denials, 1,
        "Cohere corpus must include exactly one kernel denial fixture"
    );
}
