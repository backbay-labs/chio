...
use chio_provider_conformance::{replay_groq_fixture, ReplayMode};
...
fn fixture_paths() -> Vec<PathBuf> {
    ...
    let entries = match fs::read_dir(fixture_dir()) {
        Ok(entries) => entries,
        Err(error) => panic!("read groq fixture dir: {error}"),
    };
    for entry in entries {
        let entry = match entry {
            Ok(entry) => entry,
            Err(error) => panic!("read groq fixture entry: {error}"),
        };
        let path = entry.path();
...
#[test]
fn replays_all_groq_fixtures_with_canonical_byte_assertions() {
    let paths = fixture_paths();
    assert_eq!(
        paths.len(),
        12,
        "Groq corpus must contain exactly 12 NDJSON fixtures"
    );
    ...
    for path in &paths {
        ...
        assert!(
            body.contains("\"x-groq-api-version\":\"2025-04\""),
            "{} did not pin x-groq-api-version",
            path.display()
        );
        assert!(
        ...
        let outcome = match replay_groq_fixture(path) {
            Ok(outcome) => outcome,
            Err(error) => panic!("Groq replay failed for {}: {error}", path.display()),
        };
        assert!(matches!(
            outcome.mode,
    ...
    assert_eq!(
        kernel_denials, 1,
        "Groq corpus must include exactly one kernel denial fixture"
    );
}
