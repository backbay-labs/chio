...
use chio_provider_conformance::{replay_ollama_fixture, ReplayMode};
...
fn fixture_paths() -> Vec<PathBuf> {
    ...
    let entries = match fs::read_dir(fixture_dir()) {
        Ok(entries) => entries,
        Err(error) => panic!("read ollama fixture dir: {error}"),
    };
    for entry in entries {
        let entry = match entry {
            Ok(entry) => entry,
            Err(error) => panic!("read ollama fixture entry: {error}"),
        };
        let path = entry.path();
...
#[test]
fn replays_all_ollama_fixtures_with_canonical_byte_assertions() {
    let paths = fixture_paths();
    assert_eq!(
        paths.len(),
        13,
        "Ollama corpus must contain exactly 13 NDJSON fixtures"
    );
    ...
    for path in &paths {
        ...
        assert!(
            body.contains("\"x-ollama-api-version\":\"2025-04\""),
            "{} did not pin x-ollama-api-version",
            path.display()
        );
        assert!(
        ...
        let outcome = match replay_ollama_fixture(path) {
            Ok(outcome) => outcome,
            Err(error) => panic!("Ollama replay failed for {}: {error}", path.display()),
        };
        assert!(matches!(
            outcome.mode,
            ReplayMode::Batch | ReplayMode::Stream | ReplayMode::NoToolCall
        ));
    ...
    assert_eq!(
        kernel_denials, 1,
        "Ollama corpus must include exactly one kernel denial fixture"
    );
}
