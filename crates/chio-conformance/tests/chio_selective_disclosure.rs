...
fn three_vendor_workflow_fixture_verifies_bbs_selective_disclosure() {
    ...
    let keypair = generate_bbs_keypair(b"chio-conformance-bbs-key-material-0001", b"chio").unwrap();
    ...
    assert!(
        ...
        "Chio conformance must not accept the old stub schema"
    );
...
fn three_vendor_workflow_fixture_rejects_negative_mutations() {
    ...
    let workflow = three_vendor_workflow(&ed25519);
    let projection = project_workflow_receipt_body(&workflow).unwrap();
    let keypair = generate_bbs_keypair(b"chio-conformance-bbs-key-material-0002", b"chio").unwrap();
    let signed = sign_projection(&projection, &keypair).unwrap();
    ...
    proof.disclosed[0].bytes_hex = hex::encode(b"wf-chio-forged");
    assert!(matches!(
...
fn committed_three_vendor_proof_fixture_verifies() {
    let fixture =
        include_str!("../../../examples/chio-3vendor/fixtures/selective-disclosure-proof.json");
