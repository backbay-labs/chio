...
/// Result of gating one Mistral stream payload.
...
fn parse_sse_frames(raw: &[u8]) -> Result<Vec<SseFrame>, ProviderError> {
    let text = std::str::from_utf8(raw).map_err(|error| {
        ProviderError::Malformed(format!("Mistral SSE bytes were not UTF-8: {error}"))
    })?;
...
fn parse_sse_frame(lines: &[String]) -> Result<SseFrame, ProviderError> {
    ...
    for line in lines {
        ...
        let (field, value) = line.split_once(':').ok_or_else(|| {
            ProviderError::Malformed(format!("Mistral SSE line `{line}` was missing `:`"))
        })?;
        ...
        match field {
            ...
            other => {
                return Err(ProviderError::Malformed(format!(
                    "Mistral SSE field `{other}` is not supported"
                )));
            }
        }
    }
    ...
    let data = if data_lines.is_empty() {
        ...
        Some(serde_json::from_str::<Value>(&text).map_err(|error| {
            ProviderError::Malformed(format!("Mistral SSE data was not JSON: {error}"))
        })?)
    };
...
fn ensure_streaming_allow(
    ...
    match verdict {
        ...
        VerdictResult::Allow { .. } => Err(ProviderError::Malformed(format!(
            "Mistral streaming functionCall `{}` allow verdict requested redactions; fail-closed",
            call.name
        ))),
        VerdictResult::Deny { reason, receipt_id } => Err(ProviderError::Malformed(format!(
            "Mistral streaming functionCall `{}` denied: {} (receipt {})",
            call.name,
