...
mod tests {
    ...
    fn runtime_loopback_outputs_chio_native_buyer_and_federation_schemas(
        ...
        assert_eq!(
            ...
            Some(chio_chiodos_runtime::CHIO_ATTEST_BUYER_ATTESTATION_PACKET_SCHEMA)
        );
        assert_eq!(
            read_json(&out_dir.join("cross-kernel-continuation.json"))?
                .get("schema")
                .and_then(serde_json::Value::as_str),
            Some(chio_chiodos_runtime::CHIO_FEDERATION_CROSS_KERNEL_CONTINUATION_SCHEMA)
        );
        assert_eq!(
            read_json(&out_dir.join("receipt-lineage-statement.json"))?
                .get("schema")
                .and_then(serde_json::Value::as_str),
            Some(chio_chiodos_runtime::CHIO_FEDERATION_RECEIPT_LINEAGE_STATEMENT_SCHEMA)
        );
        assert_eq!(
            read_json(&out_dir.join("receipt-lineage-bundle.json"))?
                .get("schema")
                .and_then(serde_json::Value::as_str),
            Some(chio_chiodos_runtime::CHIO_FEDERATION_RECEIPT_LINEAGE_BUNDLE_SCHEMA)
        );
        assert_eq!(
            read_json(&out_dir.join("bilateral-invocation.json"))?
                .get("schema")
                .and_then(serde_json::Value::as_str),
            Some(chio_chiodos_runtime::CHIO_FEDERATION_BILATERAL_INVOCATION_SCHEMA)
        );
