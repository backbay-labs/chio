...
mod support {
    ...
    use chio_kernel::{
        ChioKernel, DispatchIntentJournalMode, ExecutionNonceConfig, InMemoryExecutionNonceStore,
        KernelConfig, KernelError, NestedFlowBridge, ToolCallRequest, ToolServerConnection,
    ...
    impl MoneyJournalHarness {
        ...
        pub fn evaluate_priced_call(
            &self,
            request_id: &str,
        ) -> Result<chio_kernel::ToolCallResponse, KernelError> {
            let mut request = self.request(request_id);
            let preflight = self.kernel.evaluate_tool_call_blocking(&request)?;
            assert!(matches!(preflight.verdict, chio_kernel::Verdict::Allow));
            assert!(
                preflight.output.is_none(),
                "strict nonce preflight must not dispatch the priced tool"
            );
            request.execution_nonce = Some(
                *preflight
                    .execution_nonce
                    .expect("strict priced-call preflight returns an execution nonce"),
            );
            self.kernel.evaluate_tool_call_blocking(&request)
        }
...
fn priced_call_walks_journal_to_closed() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = harness.evaluate_priced_call("req-P")?;
...
fn failed_settle_leaves_the_journal_open_for_boot_reconciliation(
    ...
    let response = harness.evaluate_priced_call("req-F")?;
    assert!(matches!(response.verdict, Verdict::Allow));
...
fn tenant_scoped_money_path_stamps_journal_row_and_reconciliation_receipt(
    ...
    let preflight_context = OperationContext::new(
        session_id.clone(),
        RequestId::new("req-T-preflight"),
        agent_id.clone(),
    );
    let context = OperationContext::new(session_id.clone(), RequestId::new("req-T"), agent_id);
    let mut operation = SessionOperation::ToolCall(Box::new(ToolCallOperation {
    ...
    let preflight = harness
        .kernel
        .evaluate_session_operation(&preflight_context, &operation)?;
    let SessionOperationResponse::ToolCall(preflight) = preflight else {
        return Err("expected a tool call preflight response".into());
    };
    assert!(matches!(preflight.verdict, Verdict::Allow));
    assert!(
        preflight.output.is_none(),
        "strict nonce preflight must not dispatch the tenant-scoped tool"
    );
    assert_eq!(preflight.receipt.tenant_id.as_deref(), Some("tenant-blue"));
    let nonce = *preflight
        .execution_nonce
        .ok_or("strict tenant preflight must return an execution nonce")?;
    let SessionOperation::ToolCall(tool_call) = &mut operation else {
        return Err("expected a mutable tool call operation".into());
    };
    tool_call.execution_nonce = Some(serde_json::to_value(nonce)?);
