...
fn seed_usage(store: &dyn BudgetStore, invocation_count: u8, committed_cost_units: u64) {
    if invocation_count == 0 {
        return;
    }

    assert!(store
        .try_charge_cost("cap", 0, None, committed_cost_units, None, None)
        .test_unwrap());
    for _ in 1..invocation_count {
        assert!(store
            .try_charge_cost("cap", 0, None, 0, None, None)
            .test_unwrap());
    }
}
...
proptest! {
    ...
    fn in_memory_increment_calls_exact_projection(
        ...
        seed_usage(&store, invocation_count, 0);
    ...
    fn in_memory_charge_calls_exact_projection(
        ...
        let store = InMemoryBudgetStore::new();
        seed_usage(&store, invocation_count, committed_cost_units);
...
#[test]
fn sqlite_uncapped_saturated_counter_records_allowed_saturating_update() {
    let sqlite = SqliteFixture::new();
    let sqlite = sqlite.store();
    sqlite
        .upsert_usage(&BudgetUsageRecord {
    ...
    let actual = sqlite
        ...
        .test_unwrap();
    assert_eq!(actual, budget_increment_admits(u32::MAX, None));
    ...
    assert_eq!(events[0].usage_seq, Some(usage.seq));
    assert_eq!(events[0].invocation_count_after, u32::MAX);
}
...
fn sqlite_signed_storage_boundary_keeps_state_and_event_history_unchanged() {
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "budget arithmetic overflow: total_cost_exposed value {} exceeds SQLite INTEGER range",
            signed_max + 1
        )
    );
