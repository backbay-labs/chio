...
fn monetary_intent_carries_rail_and_authorization_id_before_dispatch(
    ...
    let mut request = harness.request("write_file");
    let preflight = harness.kernel.evaluate_tool_call_blocking(&request)?;
    assert!(
        matches!(preflight.verdict, Verdict::Allow),
        "monetary nonce preflight allows: {:?}",
        preflight.reason
    );
    assert_eq!(
        harness.invoked.load(std::sync::atomic::Ordering::SeqCst),
        0,
        "the nonce preflight must not dispatch the monetary tool"
    );
    request.execution_nonce = Some(
        *preflight
            .execution_nonce
            .expect("strict monetary preflight returns an execution nonce"),
    );
    let response = harness.kernel.evaluate_tool_call_blocking(&request)?;
    assert!(
