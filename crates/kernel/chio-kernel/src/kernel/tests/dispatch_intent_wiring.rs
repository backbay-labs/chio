...
#[test]
fn url_elicitation_exit_clears_the_journaled_intent() -> Result<(), Box<dyn std::error::Error>> {
...
fn concurrent_receipts_for_one_request_consume_once_and_append_plainly(
    ...
    let (first, second) = std::thread::scope(|threads| {
        ...
        let first = threads.spawn(move || {
            kernel
                .record_chio_receipt_consuming_optional_intent(receipt_a, Some("req-two-receipts"))
        });
        ...
        let second = threads.spawn(move || {
            kernel
                .record_chio_receipt_consuming_optional_intent(receipt_b, Some("req-two-receipts"))
        });
