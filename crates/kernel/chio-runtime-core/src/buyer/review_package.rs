...
fn verify_buyer_attestation_review_package_internal(
    ...
    let mut checks = vec![buyer_review_check(
        "chio_buyer_review.package_valid",
        true,
        "info",
    ...
    for source in sources {
        ...
        if artifact_ref.relative_path != source.relative_path {
            checks.push(buyer_review_check(
                "chio_buyer_review.artifact_path_bound",
                false,
                "error",
                &source.role,
    ...
    checks.push(buyer_review_check(
        "chio_buyer_review.artifacts_hydrated",
        true,
        "info",
        "artifact_manifest",
        None,
    ...
    checks.push(buyer_review_check(
        "chio_buyer_review.lineage_bundle_closed",
        true,
        "info",
    ...
    checks.push(buyer_review_check(
        "chio_buyer_review.lineage_bundle_bound",
        true,
        "info",
        "receipt_lineage_bundle",
        None,
    ...
    if !packet_report.accepted {
        return Ok(buyer_review_rejection_report(
            ...
            packet_report
                .failure_code
                .as_deref()
                .unwrap_or("chio_buyer_packet_hash_mismatch"),
            checks,
        ));
    }
    checks.push(buyer_review_check(
        "chio_buyer_review.packet_semantics_verified",
        true,
        "info",
    ...
    checks.push(buyer_review_check(
        "chio_buyer_review.proof_package_hydrated",
        true,
        "info",
        "proof_package",
        None,
    ...
    checks.push(buyer_review_check(
        "chio_buyer_review.package_manifest_timestamp_bound",
        true,
        "info",
        "runtime_evidence_manifest",
        None,
    ...
    checks.push(buyer_review_check(
        "chio_buyer_review.package_fresh",
        true,
        "info",
        "buyer_attestation_review_package",
        None,
    ...
    let trust_bundle_sha256 = canonical_sha256(trust_context.verifier_trust_bundle)
        .map_err(|_| ChioRuntimeError::Canonical("verifier trust bundle".to_string()))?;
    let verification_context_sha256 = canonical_sha256(trust_context.verification_context)
        .map_err(|_| ChioRuntimeError::Canonical("verification context".to_string()))?;
    ...
    checks.push(buyer_review_check(
        "chio_buyer_review.runtime_reports_bound",
        true,
        "info",
        "runtime_run_report",
        None,
    ...
    checks.push(buyer_review_check(
        ...
        None,
        None,
        "strict Chio DSSE predicate carried treaty runtime bindings",
    ));
