...
pub fn tool_args_sha256(arguments: &serde_json::Value) -> Result<String, ChioRuntimeError> {
    let bytes = canonical_json_bytes(arguments)
        .map_err(|error| ChioRuntimeError::Canonical(error.to_string()))?;
    Ok(sha256_hex(&bytes))
}

pub(crate) fn canonical_sha256<T: Serialize>(value: &T) -> Result<String, ChioRuntimeError> {
