...
fn is_runtime_pheromone_policy_schema(schema: &str) -> bool {
    matches!(schema, CHIO_RUNTIME_PHEROMONE_POLICY_SCHEMA)
}

fn is_runtime_peer_weights_schema(schema: &str) -> bool {
    matches!(schema, CHIO_RUNTIME_PEER_WEIGHTS_SCHEMA)
}

fn runtime_pheromone_policy_decision_schema(_policy_schema: &str) -> &'static str {
