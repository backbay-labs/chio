...
fn stream_bytes() -> Result<Vec<u8>, ProviderError> {
    // Groq is OpenAI-compatible. Streaming chunks emit
    // choices[].delta.tool_calls[] with `function.arguments` as a
    // JSON-encoded string.
    let payload = json!({
        "id": "chatcmpl_groq_latency",
    ...
    let body = serde_json::to_vec(&payload).map_err(|error| {
        ProviderError::Malformed(format!("Groq latency fixture encoding failed: {error}"))
    })?;
...
fn allow_verdict() -> VerdictResult {
    VerdictResult::Allow {
        redactions: vec![],
        receipt_id: ReceiptId("rcpt_groq_latency_allow".to_string()),
    }
}
...
fn run_cold_verdict_path() -> Result<(), ProviderError> {
    ...
    if gated.invocations.len() != 1 {
        return Err(ProviderError::Malformed(format!(
            "expected one Groq tool invocation, observed {}",
            gated.invocations.len()
        )));
    }
    if adapter.api_version() != GROQ_API_VERSION {
        return Err(ProviderError::Malformed(format!(
            "Groq API version drifted to {}",
            adapter.api_version()
        )));
    }
    black_box(gated);
...
fn measure_p99() -> Result<Duration, ProviderError> {
    ...
    samples.get(p99_index).copied().ok_or_else(|| {
        ProviderError::Malformed("Groq verdict latency bench produced no samples".to_string())
    })
}
...
fn cold_init_p99_stays_under_500ms() {
    let p99 = match measure_p99() {
        Ok(p99) => p99,
        Err(error) => panic!("Groq verdict latency bench failed: {error}"),
    };
    assert!(
        p99 <= COLD_INIT_P99_BUDGET,
        "Groq cold-init verdict latency p99 {:?} exceeded {:?}",
        p99,
