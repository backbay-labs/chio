...
impl SqliteRuntimeOrchestrationStore {
    ...
    pub(super) fn lock_connection(
        &self,
    ) -> Result<MutexGuard<'_, Connection>, ChiodosRuntimeError> {
...
pub(super) fn sqlite_error(error: rusqlite::Error) -> ChiodosRuntimeError {
    ChiodosRuntimeError::Store(format!("runtime orchestration sqlite: {error}"))
}

pub(super) fn sqlite_i64(value: u64, field: &str) -> Result<i64, ChiodosRuntimeError> {
...
pub(super) fn sqlite_u64(value: i64, field: &str) -> Result<u64, ChiodosRuntimeError> {
