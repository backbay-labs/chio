...
fn store_without_treaty_continuation_support_fails_closed() {
    ...
    match store.consume_treaty_continuation("continue-unsupported", "admission-1") {
        ...
        Err(error) => assert_rejected_code(error, "chiodos_treaty_continuation_store_unsupported"),
    }
...
fn assert_rejected_code(error: ChiodosRuntimeError, expected: &'static str) {
    match error {
        ChiodosRuntimeError::Rejected { code, .. } => assert_eq!(code, expected),
