...
use crate::{
    ...
    ChioRuntimeError, RuntimeEvidenceManifest, RuntimeEvidenceManifestEntry,
...
pub fn load_runtime_orchestration_evidence(
    ...
    let workflow_json = read_utf8_json_file(&workflow_report_path, "Chio runtime workflow report")?;
    let proof_json = read_utf8_json_file(&proof_report_path, "Chio runtime proof report")?;
    let manifest_json = read_utf8_json_file(&manifest_path, "Chio runtime evidence manifest")?;
    let workflow_run_report: RuntimeWorkflowRunReport =
        parse_json(&workflow_json, "Chio runtime workflow report")?;
    let proof_regeneration_report: RuntimeProofRegenerationReport =
        parse_json(&proof_json, "Chio runtime proof report")?;
    let manifest: RuntimeEvidenceManifest =
        parse_json(&manifest_json, "Chio runtime evidence manifest")?;
    ...
    let (
        ...
        let verifier_report_value: serde_json::Value =
            parse_json(&verifier_json, "Chio verifier report")?;
        let verifier_report: VerifierReport = parse_json(&verifier_json, "Chio verifier report")?;
        (
...
fn load_runtime_orchestration_role_json_artifact_hashes(
    ...
    manifest: &RuntimeEvidenceManifest,
    role: &str,
) -> Result<Option<RuntimeJsonArtifactHashes>, ChioRuntimeError> {
