...
fn chio_native_runtime_policy_material_emits_chio_decision(
    ...
    assert_eq!(metadata["admission_id"], "adm-live-1");
    assert_eq!(metadata["accepted"], false);
    assert_eq!(metadata["failure_code"], "runtime_pheromone_policy_deny");
    Ok(())
}
...
fn runtime_pheromone_query_report_parser_reads_snake_case_fields(
    ...
    let advisory = chio_runtime_core::runtime_pheromone_advisory_from_query_report_json(
        &serde_json::json!({
            "schema": "chio.pheromone.query-report.v1",
            "accepted": true,
            "concentration": {
                "subject_class": "workflow.destructive_step",
                "subject_class_namespace": "chio.runtime",
                "total_strength": 0.42,
