...
The security constraint is cross-operator boundary discipline. Federation artifacts must not create ambient runtime admission, stale trust activation, unbounded delegation, eclipse-prone quorum, or noncanonical live-money collateral before downstream kernels consume them. Federated bond collateral requires exact uppercase 3-letter currency codes so admission policies cannot canonicalize lowercase money identifiers after review. This validation lives in this crate, not in downstream marketplace or settlement callers, because federation is the first boundary that knows whether collateral is being advertised as shared operator trust material.
