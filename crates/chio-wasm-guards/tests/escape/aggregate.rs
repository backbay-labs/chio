...
fn run_class(class: &str) -> Outcome {
    ...
    match class {
        ...
        "signed_but_malicious" => {
            ...
            let message =
                signed_module_message(&module_hash, "evil-aggregate", "1.0.0", &signer_public_key);
...
fn aggregate_all_eight_escape_classes() {
    ...
    assert_eq!(tally, 8, "audit doc pins escape-class count at 8");
...
/// and forces a CODEOWNERS-gated update of the audit doc.
#[test]
fn class_taxonomy_is_pinned_at_eight() {
    assert_eq!(
        ESCAPE_CLASSES.len(),
        8,
        "escape-class count must remain 8 until the audit doc and \
         threat model are updated together"
    );
