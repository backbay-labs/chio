...
// First batch: undeclared host imports, oversized linear memory,
// fuel-budget exhaustion mid-call.
mod fuel_exhaustion;
mod oversize_memory;
mod undeclared_imports;

// Second batch: table grow / abuse, stack overflow via deep recursion,
// host re-entry abuse.
mod deep_recursion;
mod host_reentry;
mod table_grow;

// Third batch: malformed component-model encoding, signed-but-malicious
// modules.
mod malformed_component;
mod signed_but_malicious;

// Determinism gate: aggregate every escape class and pin the taxonomy
// at N=8.
mod aggregate;
