use chio_attest_buyer::{
    ...
    verify_buyer_attestation_review_package_with_legacy_verifier_json, BuyerAttestationError,
...
fn buyer_public_review_messages_use_chio_boundary_wording() {
    let lib = include_str!("../src/lib.rs");
    assert!(
        !lib.contains("existing Chiodos verifier"),
        "public buyer review messages should describe historical replay without Chiodos branding"
    );
}
...
fn chio_buyer_review_package_schema_emits_chio_review_report_schema(
    ...
    assert!(
        report
            .checks
            .iter()
            .all(|check| !check.code.contains("chiodos_buyer_review")),
        "Chio review reports must not expose historical Chiodos check codes: {:#?}",
        report.checks
    );
    Ok(())
}
...
fn chio_buyer_review_legacy_replay_api_stays_inside_attest_buyer(
    ...
    let package = BuyerAttestationReviewPackage {
        schema: "chio.attest.buyer-attestation-review-package.v1".to_string(),
        package_id: "buyer-review:packet:legacy-replay-api".to_string(),
        packet_id: "packet:legacy-replay-api".to_string(),
        buyer_id: "did:chio:buyer".to_string(),
        generated_at_unix_ms: 1_766_000_000_000,
        artifacts: Vec::new(),
    };

    let report = verify_buyer_attestation_review_package_with_legacy_verifier_json(
        &package,
        &[],
        "{}",
        "{}",
    ...
    assert!(
        report
            ...
            .iter()
            .all(|check| !check.code.contains("chiodos_buyer_review")),
        "Chio legacy-replay wrapper must keep historical internals out of public check codes: {:#?}",
        report.checks
    );
    Ok(())
}
