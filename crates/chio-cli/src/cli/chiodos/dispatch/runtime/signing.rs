...
pub(crate) fn cmd_chio_runtime_sign_trust_input(
    ...
    let body: chio_runtime::RuntimeVerifierTrustBundleV4 = serde_json::from_str(
        &read_utf8_json_file(body, "Chio runtime trust input body")?,
    )
    .map_err(|error| {
...
pub(crate) fn cmd_chio_runtime_sign_policy(
    ...
    let body: chio_runtime::RuntimePheromonePolicy = serde_json::from_str(
        &read_utf8_json_file(body, "Chio runtime pheromone policy body")?,
    )
    .map_err(|error| {
...
pub(crate) fn cmd_chio_runtime_sign_peer_weights(
    ...
    let body: chio_runtime::RuntimePeerWeights = serde_json::from_str(
        &read_utf8_json_file(body, "Chio runtime peer weights body")?,
    )
    .map_err(|error| {
    ...
    chio_runtime::runtime_pheromone_advisory_from_query_report_json(
        &serde_json::to_string(&body).map_err(|error| {
            CliError::cli_other_error(format!(
                "Chio pheromone query report validation: {error}"
            ))
        })?,
    )
    .map_err(|error| {
...
pub(crate) fn cmd_chiodos_runtime_peer_weights_hash(
    body: &Path,
    out: &Path,
) -> Result<(), CliError> {
