...
pub(crate) fn cmd_chiodos_buyer_verify(
    ...
    cmd_chio_attest_buyer_verify(package_path, trust_bundle_path, context_path, report_path)
}
...
pub(crate) fn cmd_chio_attest_buyer_verify(
    ...
    let report = chio_attest_buyer::verify_buyer_attestation_review_package_with_legacy_verifier_json(
        &package,
        &sources,
        &trust_bundle_json,
        &context_json,
...
pub(crate) fn cmd_chiodos_buyer_explain(
    report_path: &Path,
    format: &str,
    out: &Path,
) -> Result<(), CliError> {
    cmd_chio_attest_buyer_explain(report_path, format, out)
}
        ...
        check.passed
            && matches!(
                ...
                "chio_attest_buyer.review.existing_verifier_replayed"
                    | "chiodos_buyer_review.existing_verifier_replayed"
            )
    });
