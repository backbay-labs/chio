...
pub(crate) fn cmd_chio_federation_authority_issue(
    ...
    let bundle = chio_chiodos_authority::issue_authority_bundle(&profile, &request, &signing_keys)
    ...
    write_json_string(
        ...
        &chio_chiodos_authority::issuance_bundle_json(&bundle).map_err(|error| {
...
pub(crate) fn cmd_chio_federation_authority_checkpoint(
    ...
    .map_err(|error| CliError::cli_other_error(format!("Chio authority profile: {error}")))?;
    let revocations = chio_chiodos_authority::revocation_publication_request_from_json(
    ...
    write_json_string(
        out,
        &chio_chiodos_authority::signed_revocation_checkpoint_json(&checkpoint).map_err(
...
pub(crate) fn cmd_chio_federation_authority_trust_bundle_assemble(
    ...
    let workflow_intersection: chio_chiodos::WorkflowIntersectionArtifact = serde_json::from_str(
    ...
    let disclosure_policy: chio_chiodos::ChiodosDisclosurePolicy = serde_json::from_str(
    ...
    let checkpoint: chio_chiodos::SignedChiodosRevocationCheckpoint = serde_json::from_str(
