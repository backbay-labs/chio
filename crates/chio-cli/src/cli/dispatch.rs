const LEGACY_CHIODOS_CLI_ENV: &str = "CHIO_ENABLE_LEGACY_CHIODOS_CLI";

fn main() {
    if legacy_chiodos_cli_requested(std::env::args_os()) && !legacy_chiodos_cli_enabled() {
        eprintln!(
            "error: legacy `chio chiodos` compatibility commands are disabled by default; use Chio-native commands or set {LEGACY_CHIODOS_CLI_ENV}=1 for explicit migration"
        );
        std::process::exit(2);
    }
    ...
    if let Commands::Chiodos { .. } = &command {
        eprintln!(
            "error: legacy `chio chiodos` command execution is disabled; use `chio attest legacy chiodos-v1 verify` for historical verification or Chio-native commands for current artifacts"
        );
        std::process::exit(2);
    }
    ...
    let result = match command {
        ...
        Commands::Chiodos { command } => dispatch_chiodos_direct_command(command),
...
fn legacy_chiodos_cli_enabled() -> bool {
    std::env::var(LEGACY_CHIODOS_CLI_ENV)
        .is_ok_and(|value| matches!(value.as_str(), "1" | "true" | "TRUE" | "yes" | "YES"))
}

fn legacy_chiodos_cli_requested<I, S>(args: I) -> bool
where
    ...
    S: AsRef<std::ffi::OsStr>,
{
    ...
    for arg in args {
        ...
        if arg.starts_with("--") {
            skip_next_value = legacy_chiodos_global_option_takes_value(&arg);
            continue;
        }
        if arg.starts_with('-') {
            continue;
...
fn legacy_chiodos_global_option_takes_value(arg: &str) -> bool {
    if arg.contains('=') {
        return false;
    }
...
fn dispatch_chio_attest_command(command: ChioAttestCommands) -> Result<(), CliError> {
    match command {
        ...
        ChioAttestCommands::Legacy { command } => match command {
            ChioAttestLegacyCommands::ChiodosV1 { command } => match command {
                ChioAttestLegacyChiodosV1Commands::Verify {
                    package,
                    trust_bundle,
                    context,
                    report,
                } => cmd_chio_attest_legacy_chiodos_v1_verify(
                    &package,
                    &trust_bundle,
                    &context,
                    &report,
...
fn dispatch_chiodos_direct_command(command: ChiodosCommands) -> Result<(), CliError> {
    match command {
        ...
        ChiodosCommands::Authority { command } => dispatch_chiodos_authority_command(command),
        ChiodosCommands::Runtime { command } => dispatch_chiodos_runtime_command(command),
        ChiodosCommands::Treaty { command } => dispatch_chiodos_treaty_command(command),
        ChiodosCommands::Buyer { command } => dispatch_chiodos_buyer_command(command),
        ChiodosCommands::Pheromone { command } => dispatch_chiodos_pheromone_command(command),
    }
}

fn dispatch_chiodos_buyer_command(command: ChiodosBuyerCommands) -> Result<(), CliError> {
    match command {
...
fn dispatch_chiodos_authority_command(command: ChiodosAuthorityCommands) -> Result<(), CliError> {
    match command {
        ...
        ChiodosAuthorityCommands::Checkpoint {
            ...
            signing_keys,
            out,
        } => cmd_chiodos_authority_checkpoint(&profile, &revocations, &signing_keys, &out),
...
fn dispatch_chiodos_runtime_command(command: ChiodosRuntimeCommands) -> Result<(), CliError> {
    match command {
        ...
        } => cmd_chiodos_runtime_sign_pheromone_query_report(&body, &signing_seed_file, &out),
        ChiodosRuntimeCommands::Pheromone {
            command:
        ...
        ChiodosRuntimeCommands::RunLoopback {
            ...
            now_unix_ms,
            out_dir,
        } => cmd_chiodos_runtime_run_loopback(&scenario, &store_dir, now_unix_ms, &out_dir),
    }
}

fn dispatch_chiodos_treaty_command(command: ChiodosTreatyCommands) -> Result<(), CliError> {
    match command {
        ChiodosTreatyCommands::Intersect {
            ...
            now_unix_ms,
            report,
        } => cmd_chiodos_treaty_intersect(&treaty_scope, &manifest, now_unix_ms, &report),
...
fn dispatch_chiodos_pheromone_command(command: ChiodosPheromoneCommands) -> Result<(), CliError> {
    match command {
        ...
        ChiodosPheromoneCommands::Relay { command } => dispatch_chiodos_pheromone_relay_command(command),
    }
}

fn dispatch_chiodos_pheromone_relay_command(
    ...
    match command {
        ...
        ChiodosPheromoneRelayCommands::Alert { command } => match command {
            ...
            ChiodosPheromoneRelayAlertCommands::Assurance { command } => match command {
                ...
                ChiodosPheromoneRelayAlertAssuranceCommands::Retention { command } => match command
                {
                ...
                ChiodosPheromoneRelayAlertAssuranceCommands::Archive { command } => match command {
                ...
                ChiodosPheromoneRelayAlertAssuranceCommands::Closeout { command } => match command
                {
