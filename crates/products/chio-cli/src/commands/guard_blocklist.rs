...
pub(crate) fn cmd_guard_blocklist_remove(digest: &str) -> Result<(), CliError> {
    let blocklist = GuardDigestBlocklist::from_environment()
        .map_err(|e| CliError::cli_other_error(format!("failed to load guard blocklist: {e}")))?;
    let removed = blocklist
        .remove_digest(digest)
        .map_err(|e| CliError::cli_other_error(format!("failed to update guard blocklist: {e}")))?;
    if removed {
