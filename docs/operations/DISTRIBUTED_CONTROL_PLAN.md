...
- `CapabilityAuthority` in `crates/kernel/chio-kernel/src/authority.rs`
- `RevocationStore` in `crates/kernel/chio-kernel/src/lib.rs`
- `ReceiptStore` in `crates/kernel/chio-kernel/src/lib.rs`
- hosted admin surfaces in `crates/protocol/chio-mcp-remote/src/remote_mcp/`
- centralized CLI/runtime wiring in `crates/products/chio-cli/src/main.rs`
...
The service lives in `crates/platform/chio-control-plane/src/trust_control.rs` and exposes:
...
Those clients are also in `crates/platform/chio-control-plane/src/trust_control.rs`.
