...
3. Phase 3 - revive mutants-pr (files to modify: `.github/workflows/mutants.yml`: add `pull_request` trigger with paths, add the per-crate diff-touch skip step, set `GH_FUZZ_BUDGET_CAP_MODE: warn` explicitly on the PR budget step).
...
- [ ] `mutants.yml` header no longer claims a required lane or an unconfigured hard halt.
