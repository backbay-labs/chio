...
3. `RevocationEventuallyObservedDistributed` (liveness, nightly only). Under weak fairness on delivery-or-catchup for connected pairs, every revoke is eventually observed by every eventually-connected authority. Same `~>` shape as today's `RevocationEventuallySeen` (L382-408), quantifiers pushed inside the leads-to per the Apalache 0.50.1 restriction documented in the existing spec header.
