...
| `claim.finding.evidence_bound` | approved_with_scope | The cognition-market transaction passport binds its ClaimSet to a signed Finding-verifier report and the exact canonical replay-recipe attachment; role, schema, digest, and attachment substitution reject. | `runtime_qualification`, `claim_gate` |
