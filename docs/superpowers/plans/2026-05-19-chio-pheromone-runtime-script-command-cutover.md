...
query commands instead of the legacy `chio chiodos pheromone ...`
compatibility surface. Chiodos-named scripts may remain compatibility wrappers,
...
1. Capture a failing grep check showing the Chio runtime script still invokes
   legacy `chiodos pheromone receive/query` commands.
...
- [x] `rg -n -- "--(?:bin chio )?-- chiodos pheromone (receive|query)|-- chiodos pheromone (receive|query)| chiodos pheromone (receive|query)" scripts/check-chio-pheromone-runtime.sh` finds legacy invocations before implementation.
- [x] `rg -n -- "--(?:bin chio )?-- chiodos pheromone (receive|query)|-- chiodos pheromone (receive|query)| chiodos pheromone (receive|query)" scripts/check-chio-pheromone-runtime.sh` returns no matches.
