...
and write reports, but it should not call `chio_chiodos::` or
`chio_chiodos_runtime::` directly for buyer verification.
...
- [x] `cargo test -p chio-cli --bin chio chio_attest_buyer_dispatch_owns_legacy_replay_boundary` fails before implementation.
- [x] `cargo test -p chio-cli --bin chio chio_attest_buyer_dispatch_owns_legacy_replay_boundary`
- [x] `cargo test -p chio-attest-buyer --test buyer_review`
- [x] `cargo test -p chio-attest-buyer`
- [x] `cargo test -p chio-cli --bin chio chio_attest_buyer`
