...
- Modify: `scripts/check-chiodos-runtime-spine.sh`
- Create: `.github/workflows/chio-runtime-spine.yml`
- Modify: `.github/workflows/chiodos-runtime-spine.yml`
...
```bash
if rg -n 'examples/chiodos-3vendor|spec/schemas/chiodos|check-chiodos-runtime-spine' scripts/check-chiodos-runtime-spine.sh .github/workflows/chiodos-runtime-spine.yml; then
  echo "legacy runtime-spine gate still owns Chiodos implementation" >&2
  exit 1
fi
```

Expected: fail because the old script and active workflow still point at Chiodos fixtures, schemas, and script naming.
...
The packet gate already runs `chio runtime run-loopback`, validates Chio runtime and buyer schemas, explicitly verifies historical `chiodos-v1` proof packages, and rejects pending proof-regeneration markers.
...
```bash
CARGO_TARGET_DIR=/private/tmp/chio-985a-target bash scripts/check-chiodos-runtime-spine.sh --schema-only
```
...
```bash
test -x scripts/check-chio-runtime-spine.sh
if rg -n 'examples/chiodos-3vendor|spec/schemas/chiodos/v1/runtime|check-chiodos-runtime-spine' scripts/check-chio-runtime-spine.sh; then
  echo "Chio runtime-spine gate still points at Chiodos runtime implementation paths" >&2
  exit 1
fi
if rg -n 'pull_request:|push:' .github/workflows/chiodos-runtime-spine.yml; then
...
git diff --check
rg -n $'\xE2\x80\x94|\xE2\x80\x93' docs/superpowers/plans/2026-05-21-chio-runtime-spine-gate-cutover.md scripts/check-chio-runtime-spine.sh scripts/check-chiodos-runtime-spine.sh .github/workflows/chio-runtime-spine.yml .github/workflows/chiodos-runtime-spine.yml
```
