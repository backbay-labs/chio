...
**Goal:** Move the proof-package release gate from `check-chiodos-proof-package.sh` and the active `chiodos-proof-package` workflow to Chio-named ownership.

**Architecture:** `scripts/check-chio-proof-package.sh` is the active gate. It validates the committed Chio fixture tree under `examples/chio-3vendor/fixtures`, keeps the explicit legacy proof-package and verifier-report schemas where those artifact schemas remain named `chio.chiodos.*`, uses Chio federation and Chio attest schemas for renamed trust/context/negative corpus artifacts, and runs public Chio CLI verification through `chio attest legacy chiodos-v1 verify`. The Chiodos-named script and workflow may remain only as manual compatibility wrappers.
...
- Modify: `scripts/check-chiodos-proof-package.sh`
- Create: `.github/workflows/chio-proof-package.yml`
- Modify: `.github/workflows/chiodos-proof-package.yml`
...
```bash
if rg -n 'examples/chiodos-3vendor|check-chiodos-authority-issuance|generate-chiodos-proof-package|cargo test -p chio-cli chiodos|scripts/check-chiodos-proof-package.sh' scripts/check-chiodos-proof-package.sh .github/workflows/chiodos-proof-package.yml; then
  echo "legacy proof-package gate still owns active implementation" >&2
  exit 1
fi
```

Expected: fail because the old script and workflow still point at Chiodos fixture paths, Chiodos helper scripts, old generator names, and active old CI.
...
Keep legacy Chiodos schema validation only for the artifact schemas that are still explicitly named legacy proof artifacts:

```bash
spec/schemas/chiodos/v1/selective-disclosure-proof.schema.json
spec/schemas/chiodos/v1/proof-package.schema.json
spec/schemas/chiodos/v1/verifier-report.schema.json
```
...
Make `.github/workflows/chiodos-proof-package.yml` manual-only and delegate to the Chio script.
...
```bash
CARGO_TARGET_DIR=/private/tmp/chio-985a-target bash scripts/check-chiodos-proof-package.sh --schema-only
...
```bash
bash -n scripts/check-chio-proof-package.sh scripts/check-chiodos-proof-package.sh
test -x scripts/check-chio-proof-package.sh
if rg -n 'examples/chiodos-3vendor|check-chiodos-authority-issuance|generate-chiodos-proof-package|cargo test -p chio-cli chiodos' scripts/check-chio-proof-package.sh; then
  echo "Chio proof-package gate still points at Chiodos implementation paths" >&2
  exit 1
fi
if rg -n 'pull_request:|push:' .github/workflows/chiodos-proof-package.yml; then
...
git diff --check
rg -n $'\xE2\x80\x94|\xE2\x80\x93' docs/superpowers/plans/2026-05-21-chio-proof-package-gate-cutover.md scripts/check-chio-proof-package.sh scripts/check-chiodos-proof-package.sh .github/workflows/chio-proof-package.yml .github/workflows/chiodos-proof-package.yml
```
