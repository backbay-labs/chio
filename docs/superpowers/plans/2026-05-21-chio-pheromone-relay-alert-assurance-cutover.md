...
Make the active Chio pheromone relay alert-assurance fixture tree stop carrying stale Chiodos relay identifiers while preserving the historical Chiodos fixture tree and legacy runbook artifacts.
...
- Stale markers to reject in active Chio alert assurance artifacts:
  - `chiodos-relay`
  - `chiodos-pheromone-relay`
  - `CHIODOS_PHEROMONE_RELAY_RUNBOOK.md`

## Plan

1. Add a schema-only metadata gate that fails on stale Chiodos relay identifiers in the active Chio assurance tree.
...
- Do not rewrite `examples/chiodos-3vendor`.
- Do not rewrite historical signed proof packages or old compatibility docs.
- Do not rename legacy `CHIODOS_*` docs that are retained for compatibility.
