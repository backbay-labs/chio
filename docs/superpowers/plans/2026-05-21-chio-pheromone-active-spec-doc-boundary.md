...
Chio-named citations outside archive or migration notes.
...
language rather than named Chio sources.
...
`Chio`, `CHIO`, or `chio` in `spec/CHIO_PHEROMONE.md`.

- [ ] **Step 2: Run red**

Run: `bash scripts/check-chio-pheromone-transit.sh --schema-only`

Expected: failure listing the current Chio-named references in the active
spec.
