...
**Goal:** Make public `chio federation ...` dispatch route through Chio-named federation handlers while hidden Chiodos federation commands delegate for compatibility.

**Architecture:** This is a P1 command-ownership slice from `docs/architecture/CHIO_FINAL_ARCHITECTURE.md`. It does not change signed federation artifact bytes or schema IDs; it changes ownership direction so the public Chio federation path no longer calls `cmd_chiodos_*` handlers directly.
...
assert!(!body.contains("cmd_chiodos_treaty_"));
```
...
Expected: fail because `dispatch_chio_treaty_command` currently calls `cmd_chiodos_treaty_*`.
...
- [x] **Step 2: Delegate hidden Chiodos handlers**

Keep `cmd_chiodos_treaty_intersect`, `cmd_chiodos_treaty_admit`, and `cmd_chiodos_treaty_verify_packet`, but make them call the corresponding Chio-named function.
...
```bash
cargo test -p chio-cli chio_federation_treaty_dispatch_uses_chio_handlers --bin chio
cargo test -p chio-cli chiodos_treaty_verify_packet_subcommand_parses --bin chio
```
...
**Files:**
- Modify: `crates/chio-cli/src/main.rs`
- Modify: `crates/chio-cli/src/cli/chiodos/dispatch/authority.rs`
- Modify: `crates/chio-cli/src/cli/chiodos/dispatch.rs`
- Modify: `crates/chio-cli/src/cli/dispatch.rs`
...
It must also assert the extracted body does not contain `cmd_chiodos_authority_`.
...
Expected: fail because public authority dispatch still calls `cmd_chiodos_authority_*`.

- [x] **Step 3: Add Chio-named authority handlers and compatibility wrappers**

Add `cmd_chio_federation_authority_issue`, `cmd_chio_federation_authority_checkpoint`, and `cmd_chio_federation_authority_trust_bundle_assemble` as implementation owners. Keep `cmd_chiodos_authority_*` wrappers and have them delegate.

- [x] **Step 4: Route public dispatch through Chio handlers**

Update `dispatch_chio_authority_command` to call only `cmd_chio_federation_authority_*`, and add a hidden Chiodos authority dispatcher that calls the compatibility wrappers.
...
```bash
cargo test -p chio-cli chio_federation_authority_dispatch_uses_chio_handlers --bin chio
cargo test -p chio-cli chiodos_authority_issue_subcommand_parses --bin chio
```
