...
**Architecture:** The active gate must use Chio runtime schemas and public `chio runtime orchestrate ...` commands. The Chio-named script and workflow may remain only as compatibility wrappers. Legacy Chio schema references are allowed only for `runtime-orchestration-negative-fixture-corpus.schema.json` until that schema has a Chio-runtime replacement.
...
- Modify: `.github/workflows/chio-runtime-orchestration.yml`
...
```bash
if rg -n 'retired-runtime-schema-prefix|chio_runtime_orchestrate|scripts/check-chio-runtime-orchestration.sh' scripts/check-chio-runtime-orchestration.sh .github/workflows/chio-runtime-orchestration.yml; then
...
Use `spec/schemas/chio-runtime/v1-orchestration-negative-fixture-corpus.schema.json` only for the local negative corpus fixture.
...
```bash
bash -n scripts/check-chio-runtime-orchestration.sh scripts/check-chio-runtime-orchestration.sh
test -x scripts/check-chio-runtime-orchestration.sh
if rg -n 'chio\\.chio\\.runtime-(orchestration-profile|run-contract|workflow-run-report|step-evidence|proof-regeneration-report|evidence-manifest|orchestration-plan|orchestration-run-report|orchestration-status-report|orchestration-resume-plan|proof-drift-report)|chio_runtime_orchestrate|check-chio-runtime-orchestration' scripts/check-chio-runtime-orchestration.sh; then
  echo "Chio runtime orchestration gate still points at Chio runtime implementation paths" >&2
  exit 1
fi
if rg -n 'pull_request:|push:' .github/workflows/chio-runtime-orchestration.yml; then
...
git diff --check
rg -n $'\xE2\x80\x94|\xE2\x80\x93' docs/superpowers/plans/2026-05-21-chio-runtime-orchestration-gate-cutover.md scripts/check-chio-runtime-orchestration.sh scripts/check-chio-runtime-orchestration.sh .github/workflows/chio-runtime-orchestration.yml .github/workflows/chio-runtime-orchestration.yml
```
