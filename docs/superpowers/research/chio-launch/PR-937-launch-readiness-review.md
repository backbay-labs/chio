...
# SIXTH RE-REVIEW (2026-07-02) - local committed HEAD a3d5218cd (30 commits past the fifth-review baseline)

Re-ran the full launch-doc comparison after the branch advanced 30 commits past the fifth-review baseline `714d14498`. Method: two-wave multi-agent re-review (21 finder tracks total: a behavior-preservation audit of every one of the 30 commits, re-verification of the fifth review's fixed claims across all systems, and fresh-eyes completeness sweeps against every launch-doc lane), followed by dedup, a 3-lens adversarial refutation pass (evidence / mandate / novelty) on every HIGH and regression candidate, and a completeness critic. 69 agents, ~4.8M tokens. **Strict read-only at the repo level** (a separate execution agent was actively building in the tree throughout): review agents had no Edit/Write and were forbidden to run cargo/scripts; I ran only the read-only non-cargo gates myself. Cargo gates were not run locally this pass (the executor held the build); cargo-gate truth is instead anchored to green GitHub CI at the pushed head.

**Headline.** At the **pushed PR head `eea18fd9c`**, PR #937 is genuinely CI-green (69 pass / 0 fail / 2 by-design skips) and the fifth review's launch-readiness posture holds. But the branch has since accumulated **27 local-only commits** (a rust-hygiene warning tier + ~26 mechanical module-split refactors, continuing live during this pass), and this pass found four things the prior five did not: (1) **the module splits break/hollow four path-keyed gate configs** - one of them, `check-stub-surfaces.py`, is a CI merge gate that is **RED right now and will turn "Build, lint, test" red on the next push** (I confirmed this first-hand); (2) a **documentation-integrity regression** - the R-T03-17 close-out commit `9b4b62348` silently rewrote three public-settlement test assertions, which means the fifth review's "all mandated gates GREEN first-hand at 714d14498" was overstated (the `proof_verify` suite was red there); (3) genuine **launch-doc completeness gaps** five passes missed (no normative PROTOCOL.md text for the transaction-passport family; the passport root omits architecture-mandated minimum fields; the documented Proof Room quickstart fails closed from a clean checkout; bare-ACP copy on shipped surfaces outside every lint); and (4) **closure-evidence overstatements** (a runtime schema recorded as having on-disk fixtures has none; five settlement schema IDs verifier-accepted but unregistered). Net: not a security regression - every new finding is fail-closed or documentation/gate-hygiene - but the "effectively 15/15, one fixture away" posture of the fifth review is too optimistic for the **current local tree**, and pushing it as-is turns CI red.

## Empirical gate state (measured first-hand today, 2026-07-02)

| Gate | Result | Evidence |
|---|---|---|
| GitHub CI at pushed PR head `eea18fd9c` | **PASS** (69 pass / 0 fail / 2 skip) | `gh pr checks 937`; "Build, lint, test" 1h49m + "MSRV build and test" 1h24m both green; the 2 skips are by-design (external-consumer-smoke is pre-release-skipped per `conformance-matrix.yml:8-13`; notify-on-failure only fires on failure) |
| `git diff --check` | **PASS** | no whitespace errors |
| `scripts/check-chio-schema-registry.sh` | **PASS** | `OK Chio schema registry metadata` (but NOT wired into CI - see SIX-REG-CI) |
| `scripts/check-chio-proof-room-release-truth.sh` | **PASS** | `OK Proof Room release truth` |
| `python3 scripts/check-rust-file-hygiene.py` | **PASS** | 3 allowlisted oversized files expire 2026-07-31 |
| **`python3 scripts/check-stub-surfaces.py`** | **FAIL (RED) - EXIT=1** | **Confirmed first-hand.** 5 failures caused by the local module-split refactors (see SIX-STUB-01). This gate runs in CI (`ci.yml:118-125`, `ci-workspace.sh:28`), so pushing the local commits as-is turns "Build, lint, test" red. |
| `cargo build/clippy/fmt/test/xtask launch-acceptance` at local HEAD | **NOT RUN** (deliberate) | the execution agent held the build (a live `cargo build --workspace` was in flight and the tree was mutating under an active module-split campaign); disk was tight (9.7 GiB free vs a 36 GiB target dir) and concurrent cargo runs caused disk exhaustion in the fifth review. Cargo-gate truth is anchored to green CI at the pushed head; the 27 local-only commits' cargo-gate state is unverified this pass and MUST be re-run on push (see SIX-VERIFY-CI in the critic gaps). |

## Scope note - two different trees

This pass distinguishes two states that the "launch-ready" question depends on:
- **Pushed head `eea18fd9c`** (= what GitHub CI graded): green, and the fifth review's findings hold here. The three pushed commits since the baseline (`9b4b62348` max-depth fixture, `1baabde28` serve-wait, `eea18fd9c` hygiene-cap) are contained here and CI-validated.
- **Local committed HEAD `a3d5218cd`** (= the working checkout, +22 unpushed commits, plus a live uncommitted WIP): NOT CI-validated, breaks `check-stub-surfaces.py`, and carries the completeness gaps below. Everything in this section is measured against this tree unless it explicitly says "pushed head."

## Critical / High (verified first-hand; survived 3-lens adversarial refutation)

### SIX-STUB-01 - `check-stub-surfaces.py` CI gate is RED on the local tree (module split broke a path-keyed hygiene gate)
- **Status:** regression (new, introduced by local commits) - **Severity: HIGH (CI merge blocker on push)** - **confirmed first-hand (I ran the script: EXIT=1)**
- **Evidence:** commit `a88ab3d2f` renamed `crates/products/chio-cli/src/cli/session.rs` -> `session/tests.rs` and moved the `StubToolServer` fixture (with its `"stub": true` payload) into `session/test_support.rs:26`; commit `ab55e6c60` split `guard.rs`. `scripts/check-stub-surfaces.py` still keys `ALLOWLIST` and `ALLOWLIST_MATCHES` on the now-deleted paths `.../cli/session.rs` and `.../guard.rs`, and `classify()` treats `test_support.rs` as *production* (it matches none of the test-name patterns), so the `"stub": true` line is an un-allowlisted production stub hit. Running the gate yields 5 failures: (1) `session/test_support.rs:26` production stub not allowlisted; (2)+(3) `guard/new.rs:31,:49` production stub hits not allowlisted; (4) allowlist entry for deleted `cli/session.rs` points outside the scanner set; (5) allowlist entry for deleted `guard.rs` is stale. The gate runs unconditionally in CI (`ci.yml:118-125`, `ci-workspace.sh:28`).
- **Why it matters:** the CLAUDE.md house rule and prior passes treat a broken workspace/CI gate as a merge blocker. The fifth review's "GitHub CI fully green" is true only at the pushed head `eea18fd9c`, which predates these commits; pushing the local branch turns "Build, lint, test" red.
- **Fix:** rekey the two stale allowlist entries to the new paths (or teach `classify()` to treat `test_support.rs`/cfg(test)-only modules as tests), re-add the two `guard/new.rs` template strings, and re-run `check-stub-surfaces.py` + its self-test green before pushing.

### SIX-MUT-01 - Sigstore verifier split orphans the cargo-mutants examine glob (trust-boundary mutation gate silently hollowed)
- **Status:** new (introduced by local commit `e90a3fa52`) - **Severity: HIGH**
- **Evidence:** `.cargo/mutants.toml:146` still lists `examine` glob `crates/trust/chio-attest-verify/src/sigstore.rs` under "Sigstore verification boundary", but `e90a3fa52` deleted that file (split into `sigstore/{mod,core,bundle_verify,identity,parse,policy,validators,compat}.rs`, tests -> `sigstore/tests.rs`). None of the new files match any examine glob, and `exclude_globs '**/tests.rs'` additionally drops the relocated tests. Same stale path in `audits/mutation/per-crate-configs/chio-attest-verify.toml:20` and `crates/trust/chio-attest-verify/mutants.toml:16`. `cargo-mutants` does not error on a zero-match glob, so the entire signature-verification boundary (bundle/blob/bytes verify, OIDC issuer equality, SAN regex, validity window, webpki chain) drops out of the six-crate mutation gate; `releases.toml` flips this gate from advisory to blocking after two green nightly sweeps, so the activation evidence would be collected against `lib.rs` only.
- **Why it matters:** this silently hollows a hardening gate on trust-critical fail-closed code, inside a commit labeled a mechanical refactor.
- **Fix:** replace the stale path with `crates/trust/chio-attest-verify/src/sigstore/*.rs` in all three configs; add a repo check that fails when an examine glob matches zero files.

### SIX-SETTLE-TEST-01 - The R-T03-17 close-out commit silently rewrote public-settlement test semantics; the fifth review's "all gates GREEN first-hand at 714d14498" is overstated
- **Status:** documentation-integrity regression - **Severity: HIGH** (one adversarial verifier argued MEDIUM: it is fail-closed hardening + evidence-trail error, not a production regression; majority held HIGH) - **survived 3/3 refutation lenses**
- **Evidence:** `git show 9b4b62348 -- crates/products/chio-cli/tests/proof_verify.rs` (commit message: only "cover recursive swarm max-depth fixture") makes three settlement-test assertion flips: (1) `..._online_head_readback_reorg` was asserting `public settlement independent head block hash mismatch`, now asserts `loopback egress target denied`; (2) `..._trust_market_refs_without_verified_claim` flipped from assert-success to assert-failure (`public settlement trust-market context missing`); (3) `..._accepts_public_settlement_fixture` flips `assert!(contains("trust_market_context"))` to its negation. The contradicted behaviors predate the baseline (`714d14498` added unconditional `deny_loopback` in production; the "context missing" error came from pre-baseline `1b234492f`), and no production code changed between `714d14498` and `eea18fd9c` (CI green on the *new* assertions) - so the *old* assertions were necessarily RED at `714d14498`. That contradicts this doc's fifth-review lines 885/893/901 ("all mandated gates ... GREEN first-hand"), whose close-out admitted only hygiene-cap and MSRV-serve failures. At HEAD, `proof_verify.rs:1071` is the only caller of `start_public_settlement_rpc` and now asserts the *deny* path, so no test anywhere exercises a successful online independent-head RPC readback (reorg-mismatch survives only as injected-evidence unit tests in `chio-web3/src/tests.rs:1733-1749`).
- **Why it matters:** launch requirement #5 (settlement independently verified) was graded MET partly on gate evidence that was wrong for the test gate; the semantic rewrites were never recorded and the commit message misattributes them; and the CLI's JSON-RPC readback/parse path is now untested and structurally untestable end-to-end.
- **Fix:** record the true baseline test status and the settlement test-semantics changes in these docs; restore online-readback coverage via a test seam (injectable transport / test-only egress override for `CHIO_PUBLIC_SETTLEMENT_INDEPENDENT_CHAIN_RPC_URL`) exercising both readback-success and readback-mismatch; keep the loopback-deny test as an added negative.

### SIX-QUICKSTART-01 - The documented source-checkout Proof Room quickstart fails closed on a fresh checkout (undocumented trust-anchor env vars)
- **Status:** new (present since before the baseline; never recorded) - **Severity: HIGH** - **survived 3/3 refutation lenses**
- **Evidence:** `docs/start-here/PROOF_ROOM_QUICKSTART.md:10-26` gives `proof doctor` / `proof serve` / `chio-proof-room --verify-only` with zero env setup. `chio-proof-room/src/lib.rs:743-748` returns an EMPTY trusted-signer set when `CHIO_PROOF_ROOM_TRUSTED_BUNDLE_SIGNER_KEYS` is unset, and `bundle_a.rs:881-883` then rejects every signer (`proof-room.signature.signer-untrusted`); `dispatch/proof/env.rs:567-578` hard-errors when `CHIO_TRANSACTION_TRUSTED_ROOT_KEYS` is unset. All three commands hit these gated checks. Every working path sets the envs explicitly (tests `proof_doctor.rs:129`, `deploy/docker/Dockerfile:133-138`, `scripts/check-chio-transaction-passport.sh:128-131`); the bundle README documents none. The signer-untrusted error names neither the env var nor where to find the fixture keys.
- **Why it matters:** plan 07 Phase 5 exit criterion ("a reviewer can run the quickstart and inspect a valid and invalid proof") and non-negotiable 15 (first-run evidence, or the proof layer stays insider-only). Fail-closed behavior is correct, but the flagship reviewer entry-point doc is broken end-to-end from a clean environment.
- **Fix:** add the fixture trust-anchor env exports (with the checked-in fixture key hexes) to the quickstart doc or ship an env-preamble script; extend the signer-untrusted error to name the env var; add a doc-truth test that runs the quickstart commands verbatim from a clean env.

### SIX-PROTO-TXN-01 - No normative PROTOCOL.md text for the transaction-passport artifact family (mandated Phase-0 deliverable; the canonical launch proof root)
- **Status:** new completeness gap - **Severity: HIGH** (one adversarial verifier elevated it from the finder's MEDIUM to HIGH for parity with the risk/swarm equivalents) - **survived refutation**
- **Evidence:** grep over `spec/PROTOCOL.md` (3146 lines) for `transaction-passport|Transaction Passport|evidence-graph|claim-set` returns zero normative sections; the only prose mention sits inside the risk-comptroller section (`:2178`); `:321-324` only generically defers to `registry.json`. `plans/01-transaction-passport-implementation.md:34` (Phase 0 Task 1) mandates protocol text for the four `chio.transaction*` IDs. The identical gap for risk (R-T06-29) and swarm (R-T03) was recorded and fixed with dedicated PROTOCOL.md sections; the canonical launch proof root is the only major artifact family with registered schemas but zero normative protocol text.
- **Fix:** add a normative PROTOCOL.md section (mirroring the comptroller-report and swarm-admission sections) covering the passport envelope, signature/canonicalization rule, digest bindings, evidence-graph DAG rules, claim-set semantics, omission statuses, verifier-policy gates, and the 14 transaction failure codes.
- **Scope broadening (completeness critic):** the transaction-passport family is not the only one missing normative protocol text. A systematic diff of `registry.json` launch families against `spec/PROTOCOL.md` (3146 lines) found **zero** normative sections for the **commerce** (`chio.commerce.order-context.v1` + order-passport/settlement-packet/payment-lifecycle/mandate-ledger rows), **disclosure/lineage** (`chio.disclosure.capsule.v1`, `.crypto-context-report.v1`, `.leakage-ledger.v1`, `.verifier-privacy-profile.v1`, `chio.lineage.signed-subgraph.v1`), and **agent-web envelope** (`chio.agent-web-proof-envelope.v1`, `.external-projection-manifest.v1`, `.interop-verifier-report.v1`) families either. Only swarm (section 6.3.2) and risk (`chio.risk.comptroller-report.v1` at :2177) received spec text during remediation. So three more of the eight launch systems ship registered, verifier-enforced schemas with no normative protocol text, all failing the same `verification-gates.md:29` protocol alignment gate. Treat SIX-PROTO-TXN-01 as the first instance of a four-family gap; the fix should add PROTOCOL.md sections for passport, commerce, disclosure/lineage, and agent-web envelope.

## Medium (verified; new gaps or overstated closures)

- **SIX-PASSPORT-FIELDS-01 (Transaction Passport):** the passport root schema (`spec/schemas/chio-transaction/v1/transaction-passport.schema.json:7-39`, `additionalProperties:false`) omits and actively rejects the architecture-mandated minimum fields `subject`, `transaction_kind`, `trust_roots`, `artifact_refs`; `expires_at` is optional (`types.rs`, `minimal.rs:100-130` enforce the window only when present, so a never-expiring passport is mintable). `transaction_kind` has zero code hits workspace-wide. This is the unclosed half of R-T01-01, which the record now treats as fully fixed. Fix: add `subject`/`transaction_kind` (decide `expires_at` required) and validate, or record an owner-accepted deviation; mark R-T01-01 partial.
- **SIX-CLAIMSET-DIGEST-01 (Transaction Passport):** the library root-verification API (`chio-transaction-passport/src/minimal.rs:445-499`) recomputes sha256 for evidence-graph and verifier-policy bytes but for the claim set compares only two *declared* strings; `validate_claim_set_bytes` never hashes the bytes it consumes. The CLI and Proof Room loaders compensate, but `chio-control-plane/src/transaction_passport_risk.rs` forwards unhashed claim-set bytes to the vulnerable API family. No shipped binary is exploitable, but the crate's own documented fail-closed invariant ("artifact digests are recomputed from bytes") is violated for external reusers. Fix: bind `sha256(bytes) == passport.claim_set_sha256` in-library + a tampered-bytes negative test.
- **SIX-SETTLE-REG-01 (Public settlement / registry discipline):** five verifier-accepted nested schema IDs on the public-settlement path (`chio.checkpoint_statement.v1`, `chio.key-binding-certificate.v1`, `chio.web3-settlement-dispute.v1`, `chio.credit.capital-instruction.v1`, `chio.credit.capital-authority-step-proof.v1`) are absent from `registry.json` and `KNOWN_SIGNED_ARTIFACT_SCHEMAS`; the two `chio.credit.*` IDs have no JSON schema at all. Same class as the accepted CRITICAL R-T05-16, for five more IDs; the recommended nested-walk consistency test was never added. Signatures are pinned-key-verified, so this is registry discipline, not a trust hole. Fix: register the five (or adjudicate internal-only) + add the nested-walk test.
- **SIX-RT05-FIXTURE-01 (Runtime security closure evidence):** `chio.policy.activation-receipt.v1` has zero on-disk fixtures (no positive, no negative); it is constructed only in-memory in `runtime_security_passport.rs:362-397`. This doc's line 677 claims R-RT-05 has "fixtures on disk under runtime-security/valid-side-effecting-call/" - true for the attack-sim and chaos reports, false for the activation receipt. Enforcement is real; the closure text overstates fixture coverage. Fix: add positive + negative on-disk fixtures, or correct the closure text.
- **SIX-REG-CI-01 (Registry / CI enforcement):** `scripts/check-chio-schema-registry.sh` - the only gate that recomputes `MANIFEST.sha256` / registry self-hash (the RR4-ARD-01/02 fix) - is wired into NO CI workflow (zero hits across 70 workflow files + Makefile); its only caller is its own self-test, also not in CI. In-CI Rust tests check registry *membership* but never recompute the schema hashes. So the R-ARD-05/RR3-ARD-01 drift class now merges with green CI. Every prior pass ran this gate only manually. Fix: add the script + its self-test to the "Workspace structural gates" step in `ci.yml`.
- **SIX-DOCKER-CI-01 (Proof Room / Docker quickstart):** `check-chio-proof-room-docker-quickstart.sh` is a real end-to-end smoke but soft-skips (exit 0) without `CHIO_REQUIRE_DOCKER_DAEMON=1`, and nothing invokes it (no CI step, no `qualify-release.sh` reference). The shipped `release-truth.json` asserts `docker_quickstart=true` backed only by a static `docker-quickstart-evidence.json` (literal verdict "verified", no commit/timestamp/image-digest/run binding). Architecture 07's Release Truth Gate explicitly requires "Docker quickstart works from a fresh checkout." Fix: run the smoke in CI/release-qualification with the require flag; make it emit run-bound evidence and have doctor/release-truth verify the binding.
- **SIX-BLESS-PATH-01 (replay-golden bless gate):** `scripts/bless-replay-goldens.sh:153` `SOURCE_PATTERN` keys on the deleted `receipt_support.rs` monolith path (anchored `\.rs$`); commit `2a0ddadc4` split it into `receipt_support/`. The receipt-surface lane of the replay-compat bless flow is now dead and hard-refuses the documented receipt-change + `replay-compat.md` flow with a misleading exit-6. Fails closed (refuses, not silently allows). Fix: change the third alternative to the directory form and fix the usage text.
- **SIX-ACP-COPY-01 (Agent Web copy discipline, non-negotiables 12/13):** bare-ACP copy survives on shipped surfaces outside every lint: `spec/BRIDGES.md` (19 uses incl. "ACP natively supports the projection"), `docs/operations/STRATEGIC_ROADMAP.md:48,109,154` (payment/ACP-Commerce contexts), `AGENTS.md:24`, CLI `--help` (`chio-cli/src/cli/types.rs:391,649,651`), `examples/README.md`. The release-truth lint's `DEFAULT_DOCS` covers only README/docs/PROTOCOL.md and *excludes* `docs/operations/`; `AGENTS.md`, `examples/`, and CLI help are structurally out of scope. The fifth review scored #13 MET on a lint whose scope never reaches these. Fix: reword to ACP-Client/ACP-Commerce; extend the lint scope or document the exclusions; add BRIDGES.md to the standards sign-off surfaces.
- **SIX-HYGIENE-SCOPE-01 (rust-file-hygiene gate):** commit `9b4b62348` (message: only the max-depth fixture) expanded `check-rust-file-hygiene.py`'s allowlist far beyond the docs' "stale oversized-file caps" narrative: ~10 cap raises plus 7 brand-new exemptions, 4 of them production files over hard limits (`chio-swarm-authority/src/verifier.rs` 2279>2000, transaction-passport `runtime_security/artifacts.rs` 2308>2000, `chio-runtime-proof-parity` lib 1009>1000, `chio-proof-room` lib 1137>1000), none since split, all at cap with expiry 2026-07-31. Sanctioned mechanism + decomposition blueprints keep this MEDIUM. Fix: split the four before expiry; amend the roadmap to enumerate the new exemptions.

## Low (verified; behavior-preserving or bounded)

Module-split collateral (behavior unchanged, verified byte-equivalent where claimed): **SIX-KERNEL-COMMENT-01** kernel facade split (`762c9d742`) dropped the 18-line multi-tenant receipt-isolation "tenant_id is NEVER extracted from the caller-provided request" rationale block; **SIX-ARCH-DOCS-01** `chio-kernel` / `chio-control-plane` / `chio-mercury` (and wave-1's `chio-cli`) ARCHITECTURE.md module maps are stale after the splits; **SIX-QUAL-WITNESS-01** `CHIO_OPERATOR_CONTROL_SURFACE_PROFILE.json` cites the deleted `operator_report.rs` as a witness path; **SIX-SIGSTORE-COMMENT-01** (wave 1) the sigstore split dropped ~116 comment lines incl. constant-time-equality and DER-first-decode security rationale; **SIX-FMT-BLIND-01** (wave 1) `cargo fmt --all` never visits the `chio-cli` `include!`-based `main.rs` tree (autobins=false), leaving split files unformatted with no gate to catch it; plus wave-1 pheromone-relay pub(crate) widenings, a `delivery/mod.rs` `#[allow(unused_imports)]`, and a `chio-mercury` derive relocation inside "mechanical" commits.

Completeness tail (bounded, mostly guard-coverage under-pinning): **SIX-NC-PIN-01** the R-T03-17 `max-depth-exceeded` fixture is real but NOT in the launch-acceptance contract's `expected_swarm_negative_ids`, so silently dropping it would not fail the aggregate gate (one-line fix); **SIX-RISK-FLOOR-01** the "15-gate risk failure-code floor" is guarded by a test pinning only 9 of 20 shipped risk codes; **SIX-SWARM-TEST-01** two of three continuation witness-chain fail-closed paths (missing binding, digest drift) have no test anywhere; **SIX-CLAIMS-GUARD-01** the emitted-claim-is-registered guard is hardcoded to `chio-commerce-order/src/lib.rs` with no trust-market-emitter equivalent and no superset assertion (fragile under the ongoing splits); **SIX-OMISSION-01** the honest-omission path is verifier-only - zero fixtures carry `omission_policy` and the assembler has no omission reasoning (plan Phase 2 Task 4 unimplemented); **SIX-DISCLOSE-TIMING-01** the exact-timing non-disclosure rule covers only `duration_ms`, leaving wall-clock timestamp slots directly disclosable; **SIX-CANON-ADJ-01** the BAC-555 strict canonical-JSON entry points have zero callers outside `chio-core-types`; **SIX-SIGART-MIRROR-01** `built_in_signed_artifact_registry()` disagrees with `registry.json` on `introducedBy` for 4 rows (consistency test only spot-checks); **SIX-TM-COPY-01** no trust-market marketing-overclaim lint (asymmetric with the closed enterprise overclaim gate R-ENT-15).

## Refuted this pass (do NOT action)
- "Kernel/control-plane splits widened visibility beyond old effective visibility for the fail-closed drop-guard" - REFUTED (the widening does not change external API; the module is private with an unchanged pub-use list).
- "Composition alignment gate has no machine realization" - REFUTED (its required artifacts exist and the fold/split discipline is covered elsewhere).
- Dedup also dropped 6 candidates as duplicates of items already covered above (bless-path, kernel ARCHITECTURE.md, schema-registry CI-wiring, activation-receipt fixture, R-T03-17 NC-pin - all merged into their MEDIUM/LOW homes).

## 15 non-negotiables scorecard (sixth re-review)

Unchanged from the fifth review in substance at the **pushed head** (roughly 15/15). Corrections for the **local tree** and for record accuracy: **#5 (settlement)** - enforcement intact, but the only end-to-end online-readback test coverage was silently dropped and the closure evidence was overstated (SIX-SETTLE-TEST-01); **#11 (single CLI verifier + Proof Room)** - aggregate gate green, but the documented reviewer quickstart is broken from a clean checkout (SIX-QUICKSTART-01) and the max-depth NC is not pinned in the contract (SIX-NC-PIN-01); **#12/#13 (external standards / bare-ACP)** - MET only within the lint's scope; bare ACP survives on 5 shipped surfaces outside it (SIX-ACP-COPY-01); **#3 (canonical schema IDs)** - five settlement IDs verifier-accepted but unregistered (SIX-SETTLE-REG-01); **#14 (runtime authority)** - policy-activation-receipt has no on-disk fixture despite the closure claim (SIX-RT05-FIXTURE-01). Plus a cross-cutting gate-hygiene regression not tied to a single non-negotiable: **the local tree fails `check-stub-surfaces.py` and hollows the mutants + bless + schema-registry gates**, so "CI-green" is a property of the pushed head only.

## Verdict (sixth re-review)

At the **pushed head `eea18fd9c`**, PR #937 remains in the strong, CI-green state the fifth review described - nothing here reopens a fail-open security hole, and the security trajectory is still sound. But two things must be true before this is mergeable/launch-ready from the **current local tree**, in priority order:
1. **Fix the gate breakage the local refactors introduced before pushing.** `check-stub-surfaces.py` is RED right now (SIX-STUB-01, confirmed first-hand) and will turn CI red; the sigstore/mutants (SIX-MUT-01), bless-replay (SIX-BLESS-PATH-01), and schema-registry-CI (SIX-REG-CI-01) path-keyed gates are hollowed by the same module-split campaign. Re-run the full CLAUDE.md one-liner locally on the final tree - it was not run this pass.
