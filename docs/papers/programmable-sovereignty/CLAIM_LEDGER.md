...
| --- | --- | --- | --- | --- | --- |
| PS-C01 | A receiver can deny a treaty-bound request before tool dispatch. | Production-enforced | `ChioRuntimeAdmissionHook::evaluate`; `runtime_admission.rs`; `runtime_kernel_hook.rs`; live treaty-buyer closure | The configured kernel and verifier-owned store are trusted. Local requests without Chio context use the local path. | "Chio places treaty admission on the receiving kernel's pre-dispatch path." |
...
| PS-C08 | Constitutional refinement is decidable for the opaque closure model. | Blocked | `Constitution` stores `List (ReceiptId -> Bool)`; `BackwardRefines` quantifies over all receipt strings; `PredicateLang.lean` explicitly identifies the category error. | A finite list of opaque predicates makes evaluation at one receipt decidable, not universal implication between arbitrary predicates. | Do not make this claim for arbitrary closures. Use the bounded syntactic model and explicit finite domain. |
...
| PS-C10 | Backward refinement preserves already-admitted history. | Blocked | The formula is `K'(r) -> K(r)`. | The formula prevents accept-set widening. It does not require `K(r) -> K'(r)` and therefore may reject receipts formerly accepted by `K`. | "Backward refinement prevents the candidate predicate set from widening admission on the modeled domain." |
| PS-C11 | The implemented polity is the triple `(T, C, K)`. | Blocked | Lean `Polity` contains only `scope` and `constitution`; no production polity roster participates in the evaluated admission path. | Citizenship is explanatory vocabulary, not a load-bearing model or runtime component. | Use `P = (T, K)` in the submission. Discuss rosters as future governance work. |
...
| PS-C18 | The generic replay corpus evaluates bilateral treaty admission. | Blocked | `run-replay-corpus.sh` reports 50 generic replay fixtures and 20 bilateral negative cases as separate corpora. | Generic byte-equivalence fixtures remain evidence for generic replay only. | Report the two corpora separately. Do not attribute bilateral coverage to the generic 50-fixture corpus. |
...
| PS-C21 | Treaty-intersection latency is measured for `N = 1, 10, 100`. | Experimentally measured | `run-treaty-intersection.sh`; raw CSV and generated inline TeX | The hand-written harness uses the documented local profile. | Report with hardware, profile, sample count, and raw results. |
...
- **Amendment result:** "A companion Lean type carries a successful
  finite-domain no-widening check; the production runtime does not
