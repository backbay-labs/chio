...
> - Canonical type forms: `policy_hash` / `policy_digest` is hex `String` (matches existing code, RFC 8785 friendly); `tool_origin` is the canonical 4-variant enum `CallerExecuted | HostExecutedAttested | HostExecutedUnmediated | HostExecutedRedacted` per [15](15-receipt-schema-v3.md); `human_principal` is the typed enum on `CallerIdentity` (doc 14) referenced by the receipt extension (doc 15), not duplicated.
...
| [09](09-event-action-schema.md) | R3 Event actions | Unified `EventDestination` / `EventSource` with `BrokerKind` enum, not per-broker variants. | `chio.manifest.v1` to `v2` additive bump, fail-closed via ceiling negotiation, no flag-day. |
...
| [12](12-openai-responses-adapter.md) | E1 OpenAI Responses | New crate `chio-openai-responses-adapter`. **MVP: caller-executed `function` tools only over streaming SSE on non-reasoning models.** Refuses built-in-tool or reasoning requests. | Introduce `tool_origin` (canonical 4-variant enum, see TL;DR) as a core v3 field. |
...
| [16](16-latency-budget-audit.md) | X2 Latency audit | Estimated median verdict latency: **~2-4ms Ed25519-only, ~6-10ms hybrid**. Voice sub-200ms is **conditional**: yes with Ed25519 + in-process guards + async receipt write + per-bridge fast paths; no with hybrid + remote guards + sync SQLite. | Land bench stub bodies (urgent: 11+ stubs, not 4). Parallelize hybrid signing (~50-100µs savings). HTTP path does 3 signatures + 1 verify per request: voice fast-path should skip outer sign. |

## Cross-cutting threads that emerged

1. **`tool_origin` belongs on the core v3 receipt body.** Not an extension. E1, E2 (and implicitly the now-defunct R2 AGNTCY hop) all need it. Canonical 4-variant enum: `CallerExecuted | HostExecutedAttested | HostExecutedUnmediated | HostExecutedRedacted`. (`HostExecutedRedacted` covers the Bedrock Lambda trace-redaction case from E2.)
...
5. **Four new crates proposed** + 1 existing feature-flagged: `chio-directory` (consume-only), `chio-bedrock-agents-adapter`, `chio-openai-responses-adapter`, `chio-livekit-py`, plus a feature flag wrap around `chio-mcp-remote`'s AS. The previously-counted `chio-bridge-agntcy` has been struck (see erratum block). Coherent footprint, no overlap. The `chio-bridge-*` prefix is not a workspace convention; existing pattern is `-edge` (expose) / `-adapter` (consume) / `-proxy` (variant).
...
- **Receipt v3 schema** (Option D): core body promotions + extensions map + `must_understand` semantics + federation negotiation features. Includes `ActorRef` definition stub. ([15](15-receipt-schema-v3.md))
- **`EventPublish` / `EventConsume` ToolAction variants** + `chio.manifest.v2` bump. ([09](09-event-action-schema.md))
- **OAuth AS feature flag** + rename to "Chio Governed Authorization Bridge" + scope-clamp. ([07](07-oauth-as-usage-audit.md), [03](03-oauth-oidc-issuer.md))
- **`tool_origin` core v3 field** (canonical 4-variant enum; cross-cuts E1, E2). ([12](12-openai-responses-adapter.md), [13](13-bedrock-agents-bridge.md), [15](15-receipt-schema-v3.md))
- **Parallelize hybrid signing** (`crates/chio-core-types/src/pq.rs:166-170` per doc 16: verify the citation as part of this work). ~50-100µs savings on every receipt. ([16](16-latency-budget-audit.md))
...
All in `docs/research/protocol-strategy/`. Wave 1: 00-overview, 01 through 06. Wave 2: 07 through 16. Wave 3 reviews: [reviews/](reviews/). Wave 4 AGNTCY follow-up: [17-agntcy-revisited.md](17-agntcy-revisited.md). This file: `00-overview-v2.md`.
