...
| [09](09-event-action-schema.md) | R3 Event actions | Unified `EventDestination` / `EventSource` with `BrokerKind` enum, not per-broker variants. | `chio.manifest.v1` to `v2` additive bump. Requires new manifest-ceiling negotiation (`maxManifestSchema` or equivalent); today's capability ceiling does not provide that by itself. |
...
| [15](15-receipt-schema-v3.md) | X1 Receipt schema v3 | **Option D (hybrid candidate).** Promote a small core field set and route bridge / engine / surface-specific payloads through typed extensions. | Not implementation-ready until ADR settles `maxReceiptSchema`, `tool_origin` vs redaction, `extensions_hash` vs inline signing, `must_understand` registry, and hex `String` encoding for `policy_digest`. |
...
Before implementation tickets, use [18-decision-packet.md](18-decision-packet.md) to freeze receipt-v3 semantics, the boundary matrix, manifest-v2 negotiation, async receipt durability, and OAuth AS product posture.
...
- **Receipt v3 schema ADR** (Option D candidate): core body promotions + extensions map + `must_understand` semantics + federation negotiation features. Includes `ActorRef` definition stub, explicit redaction-vs-origin split, and `maxReceiptSchema` decision. ([15](15-receipt-schema-v3.md), [18](18-decision-packet.md))
...
- **Boundary classification ADR**: classify each proposed surface with `boundary_class` (`prevent`, `detect_only`, `advisory_only`, `cannot_see`) and `planning_status` (`ready_after_adr`, `blocked_by_adr`, `deferred`, `hard_skip`) before bridge plans. ([18](18-decision-packet.md))
- **Manifest v2 ADR**: decide `maxManifestSchema` / feature-bit shape, broker identity, and where event constraints are enforced. ([09](09-event-action-schema.md), [18](18-decision-packet.md))
- **`tool_origin` core v3 field candidate** (execution locus only; redaction separate by default). ([12](12-openai-responses-adapter.md), [13](13-bedrock-agents-bridge.md), [15](15-receipt-schema-v3.md), [18](18-decision-packet.md))
...
5. **Receipt / manifest negotiation**: add explicit `maxReceiptSchema` and `maxManifestSchema`, or keep feature-bit-only negotiation? PR 652 review recommends explicit ceilings unless the ADR proves feature bits are sufficient.
6. **Bench stub fix as a blocking PR**: should this be the very first commit before latency-sensitive design? Yes. It blocks latency claims for Cedar, voice, hybrid signing, and fast paths, but semantic ADRs can proceed in parallel.
