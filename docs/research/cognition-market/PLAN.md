...
> M0-M6 and M8-M10 were executed through the bite-sized plans under
> [plans/](plans/). M11 has a dark hosted listener, PostgreSQL domain and job
> storage, strict authentication, hosted SDK, deployment, worker, and release
> boundary on its implementation branch, but does not activate a public
> multi-tenant service. Exact deployed-environment network and KVM evidence
> remain release inputs, not code-completion claims. M7 remains
> conditional on verified bilateral demand and ADR-C (rule in section 6).
...
| M11 | Dark hosted production boundary | tenant-isolated PostgreSQL jobs and typed domain journals, strict hosted edge and SDKs, resumable SQLite import, remote custody, complete ACP transport, Firecracker workers, immutable deployment contracts, and signed exact-SHA canaries | M10 | dark implementation complete on branch; public traffic, M7 escrow, auctions, and full command-orchestrator promotion remain excluded; plan [plans/2026-08-28-M11-hosted-production.md](plans/2026-08-28-M11-hosted-production.md) |
