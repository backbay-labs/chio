# Chio Scarcity Economics

**Status:** Research / quantitative
**Date:** 2026-05-04 (v0.1)
**Informs:** `chio-pheromone` weighting function calibration; bounds the
attacker-budget assumption underpinning the chio safety claim
**Companions:** `docs/research/CHIO_CONCEPT.md` (section 7 hard
problem 5; honest residual on sustained majority collusion);
`spec/CHIO_PHEROMONE.md` (section 5 source diversity rules; section 6
...
residual stated in `CHIO_CONCEPT.md` section 7 is that **bilateral
...
`CHIO_CONCEPT.md` section 7 honest residual).
...
A chio passport useful for poisoning concentration queries has four
...
chio-poisoning specifically, not their gross revenue.

| Class | Plausible budget for one chio campaign | Notes / source |
|---|---|---|
...
| Nation-state (lower tier) | $5,000,000 - $50,000,000 | Mid-tier intelligence service with a dedicated cyber-influence cell. Not the top of the budget, but the realistic line for a chio-scale campaign. |
...
- The operational cost of generating semantically-plausible deposits.
  ...
  residual in `CHIO_CONCEPT.md` section 7 and is addressed by
...
The threshold safety condition from `CHIO_CONCEPT.md` is `f < 0.30`.
Solving for `A`:
...
telemetry that survives downstream verification by chio runtime
...
multiplicative reputation factor in `CHIO_CONCEPT.md` section 7 hard
problem 5).
...
### 5.1 Chio is structurally safe (substrate alone defends)
...
### 5.2 Chio is structurally unsafe (substrate cannot defend alone)
...
This is the regime where chio's *protocol* answer is correct but its
...
must accept that the trust-anchor problem (`CHIO_CONCEPT.md` section
...
"sustained majority collusion" case from `CHIO_CONCEPT.md` section 7
...
`spec/CHIO_LADDER.md` to the trust-anchor layer.
...
- **N = 4 or N = 8?** **N = 8.** Doubling the multiplier costs little in
  newcomer-onboarding latency for the realistic chio pilots and
...
`CHIO_CONCEPT.md` section 7 word-for-word.
...
`CHIO_CONCEPT.md` section 7 honest residual. Detected only if
...
- `spec/CHIO_PHEROMONE.md` v0.1, section 5 (source diversity rules),
