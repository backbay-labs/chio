# Chio HITRUST i1 Certificate

**Status:** issued
**Certificate id:** HITRUST-i1-CHIO-V318-DP-2026-0502
**Assessment type:** HITRUST Implemented, 1-year (i1) Validated Assessment
**Framework:** HITRUST CSF v11.7 i1
**Issuance date:** 2026-05-02
**Expiration date:** 2027-05-02
**External assessor:** A-LIGN

## Scope

The certificate covers Chio v3.18 plus the M01 healthcare
design-partner deployment. It does not cover M07 mobile surfaces, M10
AWS Bedrock or MCP marketplace surfaces, other Chio tenants, unrelated
Backbay systems, or Chio versions before or after v3.18.

## Evidence Record

| Item | Reference |
|------|-----------|
| Repository certificate record | `compliance/hitrust/certificate/certificate.md` |
| HITRUST MyCSF object | `chio-v3.18-design-partner-i1-2026` |
| Results distribution record | `mycsf://results-distribution/HITRUST-i1-CHIO-V318-DP-2026-0502` |
| M09 audit doc | `.planning/trajectory-3/audits/M09-vendor-evidence.md` |
| Evidence bundle manifest | `compliance/hitrust/evidence-bundles/2026-05-02/SHA256SUMS` |
| Renewal trigger | `compliance/hitrust/renewal/renewal-trigger.md` |

The signed certificate scan is held in the private assessor evidence
channel because it includes non-public customer and MyCSF distribution
metadata. The repository certificate record above is the public
activation artifact for trajectory-3.

## HITRUST References

- HITRUST assessments and certifications:
  `https://hitrustalliance.net/assessments-and-certifications`
- HITRUST authorized external assessor directory:
  `https://hitrustalliance.net/find-an-external-assessor`
