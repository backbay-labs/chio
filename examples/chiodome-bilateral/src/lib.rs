...
const OUTPUT_DIR_ENV: &str = "CHIODOME_DEMO_OUT";
const FIXTURE_SEED_ENV: &str = "CHIODOME_DEMO_FIXTURE_SEED";
...
impl OutputPaths {
    fn under(dir: &std::path::Path) -> Self {
...
impl DemoConfig {
    fn from_process() -> Result<Self, Box<dyn StdError>> {
        Self::from_sources(
            std::env::args(),
            std::env::var(OUTPUT_DIR_ENV).ok(),
            std::env::var(FIXTURE_SEED_ENV).ok(),
        )
    }
    ...
    fn from_sources(
        ...
        fixture_seed_env: Option<String>,
    ) -> Result<Self, Box<dyn StdError>> {
        let fixture_seed = parse_fixture_seed_arg(args)?
            .or(fixture_seed_env)
            .map(|seed| {
                seed.parse::<u64>().map_err(|e| -> Box<dyn StdError> {
                    format!("invalid fixture seed: {e}").into()
                })
            })
            .transpose()?;
        Ok(Self {
            out_dir: PathBuf::from(out_dir_env.unwrap_or_else(|| DEFAULT_OUTPUT_DIR.to_string())),
            fixture_seed,
        })
    }
}
...
pub fn run_from_env() -> Result<(), Box<dyn StdError>> {
    run(DemoConfig::from_process()?)
}

fn run(config: DemoConfig) -> Result<(), Box<dyn StdError>> {
    let paths = OutputPaths::under(&config.out_dir);
    println!("[chiodome-bilateral] starting demo");
    println!(
        "[chiodome-bilateral] output dir: {}",
    ...
    let (kp_org_a, kp_org_b) = match config.fixture_seed {
        Some(seed) => {
            println!(
...
mod tests {
    ...
    #[test]
    fn config_prefers_cli_seed_over_env_seed() {
        let config = DemoConfig::from_sources(
            vec![
                "chiodome-bilateral-demo".to_string(),
                "--release-fixture-seed=7".to_string(),
            ],
            Some("/tmp/chiodome-custom-out".to_string()),
            Some("42".to_string()),
        )
        .unwrap();

        assert_eq!(config.out_dir, PathBuf::from("/tmp/chiodome-custom-out"));
        assert_eq!(config.fixture_seed, Some(7));
    }

    #[test]
    fn seeded_run_writes_all_fixture_artifacts() {
        let out_dir = unique_dir("chiodome-bilateral-seeded-run");
        let result = run(DemoConfig {
            out_dir: out_dir.clone(),
            fixture_seed: Some(42),
        });
        if let Err(error) = result {
            let _ = std::fs::remove_dir_all(&out_dir);
            panic!("seeded run should succeed: {error}");
        }

        assert!(out_dir.join("receipt.json").is_file());
        assert!(out_dir.join("envelope.json").is_file());
        assert!(out_dir.join("checkpoint.json").is_file());
        let _ = std::fs::remove_dir_all(out_dir);
    }
