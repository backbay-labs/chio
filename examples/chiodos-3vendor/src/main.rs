...
fn write_pheromone_fixtures(
    ...
    let scarcity_treaty_id = scarcity_policy
        .treaty_scope
        .first()
        .ok_or_else(|| ChiodosPackageError::Json("scarcity policy has no treaty".to_string()))?
        .clone();
    ...
    let selected_policy = receiver_config
        ...
        .first()
        .ok_or_else(|| {
            ChiodosPackageError::Json("sealed runtime policy has no scarcity policy".to_string())
        })?
        .clone();
    let runtime_policy_sha256 = receiver_config
        ...
        .as_deref()
        .ok_or_else(|| ChiodosPackageError::Json("sealed runtime policy has no hash".to_string()))?
        .to_string();
...
fn seal_runtime_policy_document_hashes(
    ...
    let policies = value
        ...
        .ok_or_else(|| ChiodosPackageError::Json("missing scarcity policies".to_string()))?;
    for policy in policies {
        let reputation_epoch = policy
            .get("reputationEpoch")
            .and_then(serde_json::Value::as_u64)
            .ok_or_else(|| ChiodosPackageError::Json("missing reputation epoch".to_string()))?;
    ...
    let roots = value
        ...
        .and_then(serde_json::Value::as_array_mut)
        .ok_or_else(|| ChiodosPackageError::Json("missing verifier roots".to_string()))?;
    for root in roots {
...
) -> Result<PheromoneObservationCostVerifierRoot, ChiodosPackageError> {
...
fn signed_cost_commitment(
    ...
    runtime_policy_sha256: &str,
    generated_at_unix_ms: u64,
) -> Result<PheromoneCostCommitment, ChiodosPackageError> {
    ...
    let leaf = PheromoneObservationCostLeaf {
        ...
        treaty_id: policy
            .treaty_scope
            .first()
            .ok_or_else(|| ChiodosPackageError::Json("cost policy has no treaty".to_string()))?
            .clone(),
...
fn deposit_body_sha256(deposit: &PheromoneDeposit) -> Result<String, ChiodosPackageError> {
