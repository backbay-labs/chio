...
pub fn write_json(path: &Path, value: &Value) -> Result<()> {
    chio_example_runtime::files::replace(path, &serde_json::to_vec_pretty(value)?)?;
    Ok(())
}
...
/// The session is owned by this call, including when its future is dropped.
/// Normal completion reports close errors to the caller. Drop retains an
/// application recovery event; it cannot promise a completed tool outcome.
struct SessionLease {
    kernel: Arc<ChioKernel>,
    session: chio_core::SessionId,
    run: Run,
    request_id: String,
    closed: bool,
}
impl SessionLease {
    ...
    fn close(mut self) -> Result<()> {
        self.kernel.close_session(&self.session)?;
        self.closed = true;
        Ok(())
    }
}
impl Drop for SessionLease {
    fn drop(&mut self) {
        ...
        let cleanup = self.kernel.close_session(&self.session);
        if let Err(error) = self.run.emit(
            "call.interrupted", "host", "Call ownership ended before normal finalization",
            json!({"request_id":self.request_id,"session_id":self.session,
                "session_closed":cleanup.is_ok(),"cleanup_error":cleanup.err().map(|error|error.to_string()),
                "recovery":"Inspect the kernel receipt and protected effect before retrying"}),
        ) {
            eprintln!("Could not retain interrupted call {}: {error}",self.request_id);
        }
    }
}
