...
use chio_example_runtime::tasks::{BlockingPool, OwnedTask, TaskGroup};
...
async fn start(
    ...
    let supervisors = state.supervisors.clone();
    let cleanup = state.clone();
    let started = supervisors.spawn(async move {
        let execution_run = run.clone();
        let app = state.app.clone();
        let mut task = OwnedTask::spawn(async move { app.execute(input, execution_run).await });
        if let Ok(id) = run.id() {
            state.tasks.lock().await.insert(id, task.abort_handle());
        }
        let result = match tokio::time::timeout(std::time::Duration::from_secs(600), task.join())
            .await
        {
            Ok(Ok(result)) => result,
        ...
        let finish = state.io.run(move || retained.finish(&result)).await;
        if let Ok(id) = run.id() {
    ...
    if let Err(error) = started {
        if let Some(run) = cleanup.runs.lock().await.remove(&id) {
            let reason = error.to_string();
            cleanup
                .io
                .run(move || run.finish(&Err(anyhow::anyhow!(reason))))
                .await?;
        }
...
pub async fn main(app: impl Application) -> Result<()> {
    ...
    let state = Service {
        supervisors: TaskGroup::default(),
        io: BlockingPool::new(2),
        app,
        root,
    ...
    let signal = chio_example_runtime::shutdown_signal()?;
    let draining = state.clone();
    let served = axum::serve(listener, router)
        .with_graceful_shutdown(async move {
            signal.await;
            draining.capacity.close();
            if let Err(error) = draining.supervisors.close() {
                eprintln!("Could not close admission: {error}");
            }
            for task in draining.tasks.lock().await.values() {
                task.abort();
            }
        })
        .await;
    // The supervisors remain alive through run.finish, even after a worker is
    // cancelled. Keep the stores alive until those owners have been joined.
    let tasks = state
        .supervisors
        .drain(std::time::Duration::from_secs(15))
        .await;
    let io = state.io.drain(std::time::Duration::from_secs(15)).await;
    served?;
    tasks?;
    io?;
    Ok(())
