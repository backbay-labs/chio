...
def test_normalize_path_strips_workspace_prefixes(monkeypatch) -> None:
    ...
    cases = {
        "workspace/crates/kernel/chio-kernel/src/kernel/mod.rs": "crates/kernel/chio-kernel/src/kernel/mod.rs",
        "/workspace/crates/core/chio-core-types/src/capability.rs": "crates/core/chio-core-types/src/capability.rs",
...
def test_file_info_classifies_crate_metadata() -> None:
    info = repo_model.file_info("/workspace/crates/kernel/chio-kernel/src/kernel/mod.rs")
    assert info.path == "crates/kernel/chio-kernel/src/kernel/mod.rs"
    assert info.source_root == "crates"
    assert info.crate == "chio-kernel"
    assert info.package == ""
    assert info.kind == "code"
    assert info.nearest_manifest == "crates/kernel/chio-kernel/Cargo.toml"
...
def test_canonicality_and_generation() -> None:
    ...
    generated = repo_model.file_info("crates/core/chio-core-types/src/_generated/chio_wire_v1.rs")
...
def test_nearest_manifest_walks_to_real_cargo_toml() -> None:
    assert (
        repo_model.nearest_manifest_for_path("crates/core/chio-core/src/lib.rs")
...
def test_scoped_concepts_are_path_sensitive() -> None:
    ...
    scoped = repo_model.scoped_concepts("crates/kernel/chio-kernel/src/kernel/mod.rs", text)
