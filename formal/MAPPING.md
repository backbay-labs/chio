...
| `MonotoneLog`               | `formal/tla/RevocationPropagation.tla` (~L250) | `crates/kernel/chio-kernel/src/receipt_store.rs`                                                                      | `formal/assumptions.toml` ASSUME-SQLITE-ATOMICITY; jointly discharges RETIRED-SQLITE-CROSS-ROW | Per-authority receipt-log timestamps are strictly increasing; the log is append-only.                                           |
