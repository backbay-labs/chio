...
| `SignerPinnedHighWater` | `formal/tla/DistributedRevocation.tla` | `crates/trust/chio-federation/src/revocation_gossip.rs::RevocationRootGossip::validate_envelope`, `chio_revocation_oracle::SignedEpochRoot::verify`, `crates/kernel/chio-kernel-core/src/revocation_view.rs::RevocationView::install_if_newer` | ASSUME-ED25519 and ASSUME-SIG-CHECK; invalid frames are explicit rejected channel input | Every installed view remains authentic and at or below the genuine origin epoch, including against forged frames at already-issued epochs. |
