...
This specification defines the **chiodos governance ladder manifest**, the
...
spec named in
[CHIODOS_CONCEPT.md](../docs/research/CHIODOS_CONCEPT.md) section 4.2:
...
A chiodos participant publishes one ladder manifest per **domain**
...
3. Is intersected with the peer's manifest into a co-signed
   `chio.chiodos-ladder-intersection.v1` artefact (section 6) that gates
   all subsequent cross-trust actions in that treaty scope.
4. Is replaced only by a co-signed
   `chio.chiodos-ladder.amendment.v1` artefact (section 8).
...
The manifest schema identifier is `chio.chiodos-ladder.v1`. The full JSON
...
```json
{
  ...
  "$id": "chio.chiodos-ladder.v1",
  "title": "Chiodos Governance Ladder Manifest",
  "type": "object",
...
Per CHIODOS_CONCEPT v1.1 section 4.2 and hard problem 2, bilateral
...
`chio.chiodos-ladder-intersection.v1`. JSON Schema (Draft 2020-12):

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "chio.chiodos-ladder-intersection.v1",
  "type": "object",
  "additionalProperties": false,
  "required": [
    "schema",
  ...
  "properties": {
    "schema": { "type": "string", "const": "chio.chiodos-ladder-intersection.v1" },
    ...
    "co_signature": {
      ...
      "required": ["left", "right"],
      "properties": {
        "left": { "$ref": "chio.chiodos-ladder.v1#/$defs/signature" },
        "right": { "$ref": "chio.chiodos-ladder.v1#/$defs/signature" }
      }
    }
  },
  "$defs": {
...
| `ladder.consistency_class_mismatch` | An action class is declared with a `consistency_model` incompatible with the substrate it targets (e.g. a pheromone-deposit class declared `totally-ordered` or `quorum-required`; per `spec/CHIODOS_PHEROMONE.md` introduction, pheromones are intrinsically `crdt-commutative`). |
...
The amendment schema identifier is `chio.chiodos-ladder.amendment.v1`.

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "chio.chiodos-ladder.amendment.v1",
  "type": "object",
  "additionalProperties": false,
  "required": [
    "schema",
  ...
  "properties": {
    "schema": { "type": "string", "const": "chio.chiodos-ladder.amendment.v1" },
    ...
    "produced_at": { "type": "integer", "minimum": 0 },
    "self_signature": { "$ref": "chio.chiodos-ladder.v1#/$defs/signature" },
    "peer_co_signatures": {
      "type": "array",
      "items": {
        "type": "object",
        ...
        "properties": {
          "peer_id": { "type": "string" },
          "signature": { "$ref": "chio.chiodos-ladder.v1#/$defs/signature" }
        }
      }
    }
  },
...
- BBS+ projection of the manifest body so a peer can prove "we declare a
  ...
  CHIODOS_CONCEPT v1.1 hard problem 4.
- The `chio-pheromone` substrate spec
  ([CHIODOS_CONCEPT.md](../docs/research/CHIODOS_CONCEPT.md) section 4.1)
