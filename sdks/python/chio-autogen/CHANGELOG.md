...
- note: `chio-adapter-base` 0.1.1 ships `bind_and_redact` and the
  `DEFAULT_TOOL_POSITIONAL_NAMES` registry, but `chio-autogen` does not
  use them today (callers pass dict-keyed parameters, not a callable
  plus positional args). The dependency floor stays at
  `chio-adapter-base>=0.1.0` until a concrete consumer needs the helper.
