...
All notable changes to `chio-langchain` are documented here.
...
- feat: redact tool argument bodies before writing them to receipts. Uses
  ...
  constructor arg on `ChioTool` and `ChioToolkit`.

## [0.1.0]

- Initial release: `ChioTool` and `ChioToolkit` for wrapping Chio tool
  servers as LangChain `BaseTool` instances.
