...
case "${1:-}" in
  ...
  "--negative-only")
    ...
    ;;
  *)
    echo "usage: check-chio-pheromone-relay-alert-assurance-export.sh [--schema-only|--negative-only]" >&2
    exit 2
    ;;
esac
if [[ $# -ne 0 ]]; then
...
print("OK Chio relay alert assurance export metadata")
PY
...
cargo run -p chio-cli -- pheromone relay alert assurance verify \
...
cargo run -p chio-cli -- pheromone relay alert assurance replay \
  --bundle-dir "$GENERATED_DIR/export-bundle" \
  --trusted-exporters "$ASSURANCE_DIR/relay-alert-assurance-trusted-exporters.json" \
  --now-unix-ms "$NOW_UNIX_MS" \
...
cargo run -p chio-cli -- pheromone relay alert assurance retention plan \
...
cargo run -p chio-cli -- pheromone relay alert assurance recovery-drill \
  --bundle-dir "$GENERATED_DIR/export-bundle" \
  --trusted-exporters "$ASSURANCE_DIR/relay-alert-assurance-trusted-exporters.json" \
  --case all \
  --now-unix-ms "$NOW_UNIX_MS" \
...
if cargo run -p chio-cli -- pheromone relay alert assurance verify \
  --bundle-dir "$GENERATED_DIR/export-bundle" \
...
bash "$ROOT/scripts/check-chio-pheromone-relay-alert-assurance.sh" --schema-only
