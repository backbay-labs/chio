...
case "${1:-}" in
  ...
  "--negative-only")
    ...
    ;;
  *)
    echo "usage: check-chiodos-pheromone-relay-alert-assurance-archive.sh [--schema-only|--negative-only]" >&2
    exit 2
    ;;
esac
if [[ $# -ne 0 ]]; then
...
print("OK Chiodos relay alert assurance archive metadata")
PY
...
bash "$ROOT/scripts/check-chiodos-pheromone-relay-alert-assurance-export.sh" --schema-only
