...
case "${1:-}" in
  ...
  "--negative-only")
    ...
    ;;
  *)
    echo "usage: check-chio-pheromone-relay-alert-delivery.sh [--schema-only|--negative-only]" >&2
    exit 2
    ;;
esac
if [[ $# -ne 0 ]]; then
...
print("OK Chio pheromone relay alert delivery metadata")
PY
...
cargo run -p chio-cli -- pheromone relay alert delivery drift \
