...
for entry in catalog.get("fixtures", []):
    kind = entry.get("kind")
    if kind not in {"transaction-passport", "negative-transaction-passport"}:
        continue
...
positive_count=0
negative_count=0
while IFS=$'\t' read -r kind fixture_id passport_path; do
  case "$kind" in
    transaction-passport)
      verify_positive "$fixture_id" "$ROOT/$passport_path"
      positive_count=$((positive_count + 1))
      ;;
    negative-transaction-passport)
      verify_negative "$fixture_id" "$ROOT/$passport_path"
      negative_count=$((negative_count + 1))
      ;;
    *)
      echo "unexpected transaction-passport fixture kind: $kind" >&2
      exit 1
      ;;
  esac
done < <(
...
for entry in catalog.get("fixtures", []):
    ...
    if kind not in {"transaction-passport", "negative-transaction-passport"}:
        continue
    fixture_id = entry["id"]
    rel = pathlib.Path(entry["path"]) / "transaction-passport.json"
    if not (root / rel).is_file():
        raise SystemExit(f"missing fixture passport: {rel}")
    print(f"{kind}\t{fixture_id}\t{rel}")
PY
)

printf 'OK transaction-passport verifier gate: %s positive, %s negative\n' \
