...
case "${1:-}" in
  ...
  "--negative-only")
    ...
    ;;
  *)
    echo "usage: check-chiodos-pheromone-directory-lifecycle.sh [--schema-only|--negative-only]" >&2
    exit 2
    ;;
esac
if [[ $# -ne 0 ]]; then
...
print("OK Chiodos pheromone directory lifecycle metadata")
PY
...
cargo test -p chio-cli --bin chio chiodos_pheromone_relay_directory
cargo test -p chio-cli --bin chio chiodos_pheromone_relay_supervisor_lint_subcommand_parses

cargo run -p chio-cli --bin chio -- chiodos pheromone relay directory inspect \
...
cargo run -p chio-cli --bin chio -- chiodos pheromone relay directory promote \
...
if cargo run -p chio-cli --bin chio -- chiodos pheromone relay directory promote \
  --state "$tmpdir/peer-directory-state.json" \
...
cargo run -p chio-cli --bin chio -- chiodos pheromone relay supervisor lint \
