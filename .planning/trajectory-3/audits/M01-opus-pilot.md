# M01 Audit: Opus Cluster Design-Partner Production Pilot
...
M01 ships v3.18 to the Opus cluster (Backbay healthcare; in-house
but real workload, per D09) and observes the deployment for 30
consecutive days. The release gate is BOUNDED_OPERATIONAL_PROFILE
...
- `m01-m07-audit-handoff` (`freezes.yml` lines 154-164): opens at
  ...
  consumes the M01 Opus tenant runbook + log-export schema as
  load-bearing inputs.
...
|---------|----------|-----------|
| Opus tenant size | <single tenant>, <estimated daily receipt volume>, <Opus-side SLO targets> | Opus ops interview at P0.T1; record summary |
...
| Log-export schema fields named by Opus team | <ArcSight/CEF or QRadar/LEEF or Splunk-HEC or Elastic or generic syslog>; <fields named> | Opus SOC interview at P0.T1; record on the schema-negotiation receipt at P3.T5 |
| 30-day observation start date | <pinned at P0.T1>; no later than week 8 of W1 | Calendar pin in audit doc |
| BAA posture | <pre-existing internal Backbay BAA covering Opus + Chio> OR <fresh BAA cut for M01>; sign-off date | P0.T2 contract memo |
...
|------|-------|--------|-----------|
| | P0 contract memo signed | Opus ops team + program lead | M01.P0.T2 |
| | PagerDuty service `chio-opus-pilot-prod` provisioned | PagerDuty ops + program lead | M01.P0.T5 |
| | Tenant-onboarding rehearsal completed | Opus ops team | M01.P2.T5 |
| | Schema v1 negotiation receipt | Opus SOC team | M01.P3.T5 |
| | Week 1 incident review | Opus ops team | M01.P4.T1 |
| | Week 2 incident review | Opus ops team | M01.P4.T2 |
| | Week 3 incident review | Opus ops team | M01.P4.T3 |
| | Week 4 incident review | Opus ops team | M01.P4.T4 |
| | 30-day report published | Chio team | M01.P4.T5 |
| | Opus tenant ops sign-off memo received | Opus ops team | M01.P5.T2 |

## 4. PagerDuty service-naming + on-call rotation contract

[TODO M01 milestone agent fill at P0.T5:]

- **PagerDuty service name:** `chio-opus-pilot-prod`
- **Routing key owner:** <Chio team account | Opus ops team account>
...
- **On-call rotation cadence:** <weekly | bi-weekly>; rotation
  members from Opus ops + Chio kernel team.
...
- **Chio mediation edge placement:** <sidecar process | in-process
  library | wrapped MCP edge> relative to `opus_api`
  (`clusters/opus/apps/opus/services/api/`, default port 8103 per
  `clusters/opus/CLAUDE.md`).
...
- **Retention contract:** Opus deployment retains receipts for 6
  years on its own audit-store per HIPAA. Chio does not ship a
  long-retention path in M01.
- **Schema-negotiation receipt:** Opus SOC team accepted v1 on
  <date>; sign-off captured under section 3 evidence log.
...
- **Bounded-profile-hold attestation (P4.T6):** "30-day observation
  ...
  evidence held under Opus production load."

## 9. Closure attestations

[TODO M01 milestone agent fill at P5 close:]

- Opus tenant ops sign-off memo: <link / hash> (P5.T2)
...
- Decisions: D09 (Opus design partner), D15 (7-day freshness).
