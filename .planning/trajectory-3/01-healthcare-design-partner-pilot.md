# Milestone 01: Healthcare Design-Partner Production Pilot
...
release without one. A single healthcare design partner (selected
during M01.P0/P1 scoping per D09) is real workload run by a separate
ops team, which makes onboarding tractable inside the Wave 1 window
while still satisfying the BOUNDED_OPERATIONAL_PROFILE release gate
...
3. The audit-log export surface ships JSON-shaped exporters but no
   ...
   the design-partner SOC is QRadar-shaped) and pins the
...
the design-partner tenant runbook + schema v1 as the design-partner
reference) and M09 HITRUST i1 assessment (which scopes the
assessment to the design-partner deployment per D02). Both freezes
(`m01-m07-audit-handoff` and `m01-m09-audit-handoff` in
`freezes.yml`) point at this milestone's audit doc.
...
- The quota lane is capacity-tested under real design-partner traffic
  but the underlying budget store is NOT re-shaped; the bound stays
...
- `PagerDutyBackend` source code is not edited; M01 plumbs a real
  routing key through the design-partner deployment configuration
  and adds a weekly heartbeat workflow at
  `.github/workflows/healthcare-pilot-pagerduty-heartbeat.yml`.

Design-partner identity is NOT bound in trajectory-3 docs. The
healthcare design partner is selected during M01.P0/P1 scoping from
a candidate pool (regional payer, digital-health startup with
existing BAA-ready posture, AI-driven underwriting platform,
telehealth network) by end of week 1. Selection criteria: BAA-ready
in <=4 weeks, has a real production workload, willing to publish
ops sign-off memo + 30-day incident report. Decision register
reference: D09.
...
- Does not promote multi-tenant. M01 is single-tenant per verdict
  and per the design-partner contract; the runbook declares
  "single-tenant deployment" so the bounded profile claim is honest.
- Does not chase a substitute design partner if the selected partner
  withdraws after scoping. Halt trigger 12 (design-partner
  withdrawal) fires; the verdict accepts single-tenant. Candidate
  pool depth (>=3 candidates pre-scoped by week 0) is the up-front
  mitigation.
...
`.planning/trajectory-3/audits/M01-healthcare-pilot.md` section 2.

- Design-partner tenant size at P0: estimated tenants (single tenant
  per D09), daily receipt volume (TBD: collect from the design-partner
  ops team; gates P2 quota lane sizing), current design-partner-side
  SLO targets (TBD: collect at P0.T4 once topology and the
  partner's existing API service surface are pinned; Chio
  mediation-edge SLOs are net-new in M01).
...
- Log-export schema fields the design-partner team has named as
  required: TBD at P0.T1 from the design-partner SOC interview (P0
  question: ArcSight/CEF, QRadar/LEEF, Splunk-HEC, Elastic, or
  generic syslog). v1 schema MUST cover JSON canonical (already
  shipped via `OcsfExporter`) plus exactly one of CEF or LEEF; the
  recommended default is CEF per RESEARCH.md.
...
- `crates/chio-siem/Cargo.toml`: P3.T2 may pin a `syslog`
  RFC-5424 framing crate if the design-partner SOC requires syslog
  framing rather than newline-delimited CEF. Decision deferred to
  P3 schema-negotiation receipt; current presumption is that
...
- **PagerDuty.** Real service named `chio-healthcare-pilot-prod`
  with an Events API v2 integration key. Routing-key ownership
  decision (Chio team account vs the design-partner ops team
  account) recorded in audit doc evidence log at P0.T5.
- **Design-partner identity provider.** Whatever auth the partner's
  existing API surface uses; Chio-side hosted auth runs as
  `chio mcp serve-http --auth-token` or `--auth-jwt-public-key`
  per `OPERATIONS_RUNBOOK.md` lines 41-50. P0.T4 topology diagram
  pins which.
- **Design-partner log sink.** SIEM endpoint per the partner's SOC
  pipeline (P0 interview question; CEF target if ArcSight, LEEF
  target if QRadar, Splunk HEC URL if Splunk, Elasticsearch URL if
  Elastic). Receives audit-log export per the v1 schema landing
  in P3.
...
- Real PagerDuty integration with on-call rotation matched to the
  design-partner ops team. PagerDuty service named
  `chio-healthcare-pilot-prod` (P0.T5 contract memo, P1.T4
  plumbing). Heartbeat workflow at
  `.github/workflows/healthcare-pilot-pagerduty-heartbeat.yml` (P1.T6).
- Sustained-load runner under `bench/healthcare-pilot-capacity/`
  (new crate, sibling to `bench/ttfrh/`). Runner replays
  design-partner shadow traffic at 1x / 2x / 5x and records
  p50/p95/p99 latency, receipt-write throughput, trust-control
  state-machine convergence time, and `chio-siem` exporter
  backpressure. (P2.T1)
...
- Log-export schema v1 negotiated with the design-partner team and
...
- Closure: design-partner ops sign-off memo, runbook live URL,
  schema v1 path, four success-criteria rows green, both freezes
  closed (P5.T1..T5).
...
- Multi-tenant beyond the selected design partner. Single-tenant
  pilot is the verdict scope; the runbook declares this explicitly
  so the bounded-profile claim is honest.
...
- BAA negotiation outside the design-partner posture. P0.T2
  records whether a pre-existing BAA covers the design partner +
  Chio or whether one must be cut; the engineering effort to
  author a BAA is owned by program-lead and legal, not M01.
- A second design-partner backup pursued in parallel after week 1
  selection. The verdict accepts single-tenant; if the selected
  partner withdraws after scoping, halt trigger 12 fires. Candidate
  pool depth (>=3 candidates pre-scoped by week 0) is the up-front
  mitigation, not an active second pilot.
...
- Receipt long-retention storage tier. The design-partner deployment
  retains Chio receipts for 6 years on its own audit-store per
  HIPAA per P3 retention contract; Chio does not ship a
  long-retention path in M01.

## Phases

### P0 - Audit doc + design-partner contract + tenant onboarding plan

Wave-opener for M01. Goal: open the milestone audit doc with hard
counts, select a design partner from the candidate pool and sign
a written contract naming the design-partner relationship under
D09 + D15, and stand up the two operator-runbook stub files that
subsequent phases extend.

- M01.P0.T1 - Open audit doc and fill four hard-count rows.
- M01.P0.T2 - Healthcare design-partner contract memo (selection
  from candidate pool + BAA posture + D09 ratification) recorded
...
- M01.P0.T4 - Production deployment topology diagram naming the
  Chio mediation edge placement relative to the partner's API
  surface (sidecar process / in-process library / wrapped MCP
  edge) at `docs/operator-runbook/topology.md`.
...
against the design-partner on-call. Six files land under
...
Goal: prove BOUNDED_OPERATIONAL_PROFILE holds at design-partner
production load and capture quota lane sizing. New sibling crate
`bench/healthcare-pilot-capacity/`.

- M01.P2.T1 - Sustained-load runner crate scaffold under
  `bench/healthcare-pilot-capacity/`.
- M01.P2.T2 - Shadow-traffic capture script at
  `bench/healthcare-pilot-capacity/scripts/shadow-capture.sh`.
...
### P3 - Audit-log export schema v1 negotiated with design-partner team
...
- M01.P3.T2 - CEF emitter at
  `crates/chio-siem/src/exporters/cef.rs` (or LEEF if the
  design-partner SOC is QRadar-shaped per P0 interview).
...
- M01.P3.T4 - Schema-linter CI job at
  ...
  + CEF golden file.
- M01.P3.T5 - Schema-negotiation receipt (design-partner team
  accepted v1) recorded in audit doc evidence log.
...
### P5 - Design-partner ops review attests bounded operational profile
...
- M01.P5.T2 - Design-partner ops sign-off memo recorded under closure
...
- M05 `pii_phi_exposure` advisory closure is a precondition for
  design-partner PHI handling confidence; M01 references M05's
...
- M07 chio-kernel-mobile MVP consumes the design-partner tenant
  runbook + schema v1 as load-bearing inputs (per `freezes.yml`
  `m01-m07-audit-handoff` and README invariants table). The
  design-partner mobile patient-app is the M07 design-partner
  surface per D09 consequences.
...
1. **Design partner declines after scoping** (halt trigger 12). The
   selected design-partner ops team declines to continue, BAA
   negotiations stall, or organizational reprioritization fires.
   Mitigation: candidate pool depth (>=3 healthcare candidates
   pre-scoped by week 0); selection contracted in P0.T2 with a
   written memo; if the selected partner withdraws after scoping,
   halt trigger 12 fires (the verdict accepts single-tenant per
   D09 and re-scoping to a fallback candidate from the pool requires
   user authorization, not silent substitution). If withdrawal
   happens inside the 30-day window, the affected weekly review
   ticket captures the withdrawal date and the orchestrator pauses
   M01 and surfaces to user.
2. **PagerDuty integration latency.** PagerDuty service provisioning
   ...
   the design-partner ops team can stretch. Mitigation: P0.T5 names
   the service in the contract memo; P1.T4 plumbs the routing key
   the same week. The heartbeat workflow at P1.T6 verifies the
   wiring stays warm for the duration of the observation window.
...
4. **PHI exposure in receipts.** A `ChioReceipt` carries
   ...
   `decision.reason`, and guard-evidence text. If the design partner
   passes patient identifiers as tool arguments, those land in
   receipts unless
...
6. **BAA timing.** HIPAA Business Associate Agreement between Chio
   team and the design-partner tenant is a precondition for any
   deployment that processes PHI. Selection criteria require
   BAA-ready in <=4 weeks. Mitigation: P0.T2 records BAA posture
   (pre-existing BAA covering both parties, or a fresh BAA cut
   for M01); engineering work proceeds independently because Chio
   cannot leak PHI in receipts whether or not a BAA exists, but
   no production traffic flips on until BAA posture is documented.
7. **Receipt retention vs HIPAA 6-year retention.** Chio receipts
   ...
   doc names the retention contract; the design-partner deployment
   retains Chio receipts for 6 years on its own audit-store. Chio
   does not ship a long-retention path in M01; if the design partner
   cannot retain locally, record as M09 follow-up.
...
- Design-partner ops sign-off memo committed under
  `.planning/trajectory-3/audits/M01-healthcare-pilot.md` within 7
  days of P5 close (D15 freshness window).
...
- PagerDuty service `chio-healthcare-pilot-prod` is live; the
  weekly heartbeat workflow has fired at least four times without
...
- BOUNDED_OPERATIONAL_PROFILE attestation row in audit doc closure
  ...
  design-partner production load."
