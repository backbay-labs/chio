# M01: Opus Cluster Design-Partner Production Pilot

**Wave:** W1  |  **Trust-boundary:** yes  |  **Tickets:** 32  |  **Effort weeks:** 4/6/9 (low/real/high)

## In one paragraph

M01 ships v3.18 to a real production tenant (the Opus cluster, Backbay
healthcare; in-house but real workload, per D09) and observes the
deployment for 30 consecutive days. The release gate is
BOUNDED_OPERATIONAL_PROFILE: the Opus tenant ops review attests the
bounded profile holds under real workload. Implementation lands a
tenant-shaped operator runbook under `docs/operator-runbook/`, plumbs
a real PagerDuty hookup against Opus on-call (the
`crates/chio-siem/src/alerting.rs` `PagerDutyBackend` is read-only
consumed; M01 names the routing key and rotation), ships an
audit-log export schema v1 at `spec/audit-log/export-schema.v1.json`
plus a new CEF emitter at `crates/chio-siem/src/exporters/cef.rs`,
and sustains a capacity-tested quota lane sized to Opus production
load via a new `bench/opus-capacity/` crate. Two audit-doc freezes
(`m01-m07-audit-handoff`, `m01-m09-audit-handoff`) point at this
milestone.
...
|-------|-----------|---------|-------------|
| P0 | Audit doc + Opus contract + tenant onboarding plan | 5 | 4.5 |
...
| P3 | Audit-log export schema v1 negotiated with Opus team | 5 | 5.5 |
| P4 | 30-day production observation + week-by-week incident review | 6 | 3.5 |
| P5 | Opus tenant ops review attests bounded operational profile | 5 | 3.0 |
| **Total** | | **32** | **30.5** |
...
- M01.P0.T2 (0.5d) - Opus design-partner contract memo + BAA posture
...
- M01.P5.T2 (1.0d) - Opus ops sign-off memo recorded under closure attestations
...
- D09 - Opus cluster is the M01 design partner (in-house, real workload)
...
- Opus cluster ops sign-off memo received within 7 days of P5 close.
