# Chiodos Long Horizon Refactor Goal
...
### 1. Decompose `chio-chiodos-runtime`

Split `crates/chio-chiodos-runtime/src/lib.rs` into real modules with clear responsibilities, preserving public re-exports where needed:

- `schema`: schema IDs, schema hash constants, schema manifest helpers.
- `error`: runtime error types and conversions.
- `types`: core Chiodos domain structs and enums.
...
`crates/chio-cli/src/cli/dispatch.rs` should parse args, read and write files, call runtime APIs, and render output. It should not own Chiodos security logic.

Move reusable invariants into `chio-chiodos-runtime`, especially:
...
- Split Chiodos command handling into dedicated CLI modules if the project pattern allows it.
...
Audit Chiodos proof and buyer paths for ad hoc `.get()`, string field probing, loosely typed `serde_json::Value`, and duplicated hash checks.
...
- Ensure governed calls cannot bypass Chiodos admission.
...
Cross-check Chiodos scarcity docs against code.
...
Review `spec/schemas/chiodos/v1`.
...
The current Chiodos runtime test surface is too large.
...
Review `chio-chiodos-loopback` and the CLI dependency on it.
...
- `docs/research/CHIODOS_CONCEPT.md`
- `docs/research/CHIODOS_SCARCITY_ECONOMICS.md`
...
- `cargo clippy -p chio-chiodos-runtime -p chio-cli -p chio-federation -p chio-kernel --all-targets -- -D warnings`
- `cargo test -p chio-chiodos-runtime`
- `cargo test -p chio-cli`
- `cargo test -p chio-federation`
- relevant `chio-kernel` tests
- Chiodos runtime spine script
- Chiodos live treaty buyer closure script
...
- No actionable P0/P1/P2 Chiodos architecture, security, or correctness gaps remain.
- Runtime monolith is materially decomposed into coherent modules.
- CLI no longer owns core Chiodos validation logic.
