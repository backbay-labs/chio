# Chio Runtime Modularization Wave 2 Goal
...
5. Inspect current local module sizes with `wc -l` for:
   - `crates/chio-runtime-core/src/lib.rs`
   - `crates/chio-runtime-core/src/*.rs`
   - `crates/chio-cli/src/cli/dispatch.rs`
   - `crates/chio-cli/src/cli/types.rs`
   - `crates/chio-runtime-core/tests/*.rs`
...
- `src/admission_hook.rs`: `ChioRuntimeAdmissionHook`, kernel metadata, reservation/release behavior.
- `src/request_context.rs` or equivalent: typed parsing for Chio admission/treaty context embedded in governed intents.

Requirements:

- Preserve pre-dispatch fail-closed behavior.
- Ensure federated Chio calls cannot bypass treaty context.
...
- Parse proof packages through `chio_attest_buyer_core` typed APIs where possible.
...
`cmd_chio_runtime_run_loopback` remains a large local harness inside CLI dispatch. It owns scenario parsing, tool server setup, deterministic key material, treaty context construction, proof generation, parity checks, buyer packet creation, and artifact writing.

Fix direction:

- Move loopback execution into `chio-attest-loopback` or a dedicated harness module/crate.
...
`crates/chio-cli/src/cli/dispatch.rs` and `types.rs` still contain too much Chio-specific command surface.

Fix direction:

- Split Chio dispatch into dedicated CLI modules if the current CLI include pattern permits it:
  - `cli/chio/runtime.rs`
  - `cli/chio/treaty.rs`
  - `cli/chio/buyer.rs`
  - `cli/chio/pheromone.rs`
  - `cli/chio/authority.rs`
  - `cli/chio/loopback.rs`
...
Chio shell gates are large and contain complex fixture generation. They should orchestrate checks, not become hidden implementations of runtime behavior.
...
- `cargo clippy -p chio-runtime-core -p chio-cli -p chio-attest-loopback -p chio-federation -p chio-kernel --all-targets -- -D warnings`
...
- additional Chio scripts touched by the refactor
...
- No actionable P0/P1/P2 Chio architecture, security, or correctness gaps remain in the touched areas.
...
- CLI Chio command code is smaller and less duplicated.
