# Chiodos P3 Modular Polish Goal
...
The Chiodos P0, P1, and P2 behavior work is treated as closed unless a fresh
...
Finish the next Chiodos architecture cleanup pass:

1. Split the remaining large Chiodos pheromone CLI dispatch and type files into
   focused modules.
...
4. Preserve all current Chiodos fail-closed behavior and regression coverage.
...
`crates/chio-cli/src/cli/chiodos/dispatch/pheromone.rs` is about 2156 lines and
...
- `crates/chio-cli/src/cli/chiodos/dispatch/pheromone.rs`
  - Keep as a small aggregator and command router.
- `crates/chio-cli/src/cli/chiodos/dispatch/pheromone/runtime.rs`
  - Receive and query.
- `crates/chio-cli/src/cli/chiodos/dispatch/pheromone/relay.rs`
  - Relay lint, serve, enqueue, tick, catchup, status, observe, metrics, and
    trend.
- `crates/chio-cli/src/cli/chiodos/dispatch/pheromone/alerts.rs`
  - Alert evaluate, handoff, normalize, and review.
- `crates/chio-cli/src/cli/chiodos/dispatch/pheromone/delivery.rs`
  - Delivery import, ack, drift, and window handling.
- `crates/chio-cli/src/cli/chiodos/dispatch/pheromone/assurance.rs`
  - Assurance package, export, verify, replay, retention, recovery, archive,
    and closeout.
- `crates/chio-cli/src/cli/chiodos/dispatch/pheromone/directory.rs`
  - Directory inspect, promote, reject, supervisor lint, and peer directory
    helpers.
- `crates/chio-cli/src/cli/chiodos/dispatch/pheromone/io.rs`
...
`crates/chio-cli/src/cli/chiodos/types/pheromone.rs` is about 984 lines and
...
`crates/chio-chiodos-runtime/src/lib.rs` is about 2797 lines. It already has
...
`crates/chio-chiodos-runtime/tests/runtime_admission.rs` is about 2702 lines.
...
`scripts/check-chiodos-runtime-spine.sh` is about 825 lines and still writes
...
- Move static fixture JSON into a checked-in fixture directory, preferably under
  `examples/chiodos-3vendor/fixtures/runtime-spine/` unless the runtime harness
...
- Preserve all Chiodos runtime, treaty, buyer, economics, scarcity, DSSE,
...
cargo clippy -p chio-cli -p chio-chiodos-runtime -p chio-chiodos-runtime-harness -p chio-chiodos-loopback -p chio-federation -p chio-kernel --all-targets -- -D warnings
cargo test -p chio-chiodos-runtime
cargo test -p chio-chiodos-loopback
cargo test -p chio-cli chiodos_
cargo test -p chio-federation strict_chiodos_treaty_review_binds_live_material --lib
cargo test -p chio-kernel chiodos_runtime_admission --lib
...
- The listed P3 modularity issues are closed or explicitly deferred with a
  concrete reason.
- No fresh P0, P1, or P2 Chiodos behavior issue is discovered.
