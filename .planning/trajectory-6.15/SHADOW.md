# Chio 6.15 Shadow

Candidate focus: downstream alert delivery hardening only if local delivery evidence import surfaces reveal a concrete operator gap.

Possible scope:

- downstream exporter normalization for common SIEM and Alertmanager evidence drops
- longer window delivery drift summarization
- route-owner handoff review packets
- recovery drills for missing delivery evidence

Out of scope until explicitly promoted:

- credentialed live notification dispatch from Chio
- storing downstream secrets or dynamic sink URLs
- dynamic trust or peer discovery
- policy mutation from alert state
- new transports, settlement, hidden predicates, VC Data Integrity BBS, zkVM, or FROST
