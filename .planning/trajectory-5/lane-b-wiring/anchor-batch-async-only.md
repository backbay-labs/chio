...
Implementation polish (regex tightening, multi-line policy struct support) is a B3.3 PR detail. **A future trj6 ticket may upgrade this to AST-based analysis (e.g. `syn`-parsed call-graph traversal in a Cargo `xtask`) for a true static guarantee, but that is OUT OF SCOPE for trj5.** Lane B3's contract is honest: the runtime gate is the MUST enforcement; the lint is documentation.
