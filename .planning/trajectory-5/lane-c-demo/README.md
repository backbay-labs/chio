...
selective-disclosure auditor view.
...
2. **Receipt v2 fail-closed under negotiated v2** - the dual-signed
   receipt body is the underlying `ChioReceipt`; Lane B `TRJ5-B2.x`
...
makes the `v0.1.0-bounded-chiodome` release credible. If the floor
banner still reads 31% kill, the bounded-claim language has to
...
|---|---|---|---|---|
| `verify_capability_full` is the only hot-path verifier | B | Single-entry verifier | TRJ5-B1.6 | synthesis lines 100-103 |
| Receipt v2 fails closed when negotiated | B | Receipt v2 hot path | TRJ5-B2.5 | synthesis lines 104-107 |
| Anchor-batch async only when witness required | B | Anchor-batch enforcement | TRJ5-B3.5 | synthesis lines 108-111 |
| `ToolServerConnection` -> `async_trait` | B | Architectural prerequisite | TRJ5-B0.5 | synthesis lines 96-100 |
| DSSE-conformant bilateral signing | B | Sub-lane B4 (added W3 per R4 Finding 1) | TRJ5-B4.5 | `lane-b-wiring/dsse-bilateral-signing.md` |
...
7. The continuous CI workflow `chiodome-demo-continuous.yml`
   (TRJ5-C6.3) has been green for 7 consecutive nights before the
   tag goes out.
...
- `tickets.md` - concrete tickets `TRJ5-C1.x..C6.x`
