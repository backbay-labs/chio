...
## TRJ4-015 - `RevocationCutCompleteness` rewrite (TRJ5-A4.2)
...
(captured as TRJ5-A4.2 sub-task).
...
TRJ5-A4.2 includes a feasibility-spike sub-task: write a 20-line TLA
...
`audits/evidence/TRJ5-A4.2/feasibility-spike.md`.

If Apalache 0.50.x does not handle the encoding, TRJ5-A4.2 escalates.
...
## TRJ4-016 - Split `Allow` into `LogReceipt` + `PublishAllow` (TRJ5-A4.1)
...
Trace length 6 (matches the bumped `EpochMax` from TRJ5-A4.3). The
property is provable under apalache safety tier, no temporal lane
needed.

## TRJ4-017 - Bump `EpochMax` from 4 to 6 (TRJ5-A4.3)
...
TRJ5-A4.3 records the apalache run wall-clock BEFORE and AFTER the
bump, captured in `audits/evidence/TRJ5-A4.3/length-budget.md`. If the
...
## TRJ4-018 - `RevocationEventuallySeen` apalache fix + temporal-lane promotion (TRJ5-A4.4)
...
- The workflow is added to required-checks for `main` (configured via
  the GitHub branch-protection rule, captured in the TRJ5-A4.4 PR).
...
TRJ5-A4.3 lands). Two consecutive green runs captured to
`audits/evidence/TRJ5-A4/temporal-lane-runs.md`.

## Advisory-to-required promotion plan

1. **TRJ5-A4.1 lands**: split actions and new property in
   ...
   `INVARIANT` clause).
2. **TRJ5-A4.2 lands**: `RevocationCutCompleteness` provable at depth
   3.
3. **TRJ5-A4.3 lands**: `DEPTH_MAX = 6` propagation; both cfg files
   updated.
4. **TRJ5-A4.4 lands in two phases**:
...
5. **TRJ5-A4.5**: cascade theorem-inventory updates if any
...
theorem-inventory updates." TRJ5-A4.5 is the cascade-update ticket.
Inputs:
...
tautologically again. TRJ5-A4.5 reviews `theorem-inventory.json` AND
...
- An `apalache-temporal.yml` job that still carries `continue-on-error:
  true` after TRJ5-A4.4 lands fails the close bar.
