...
**Authoritative source**: #620 tree listing plus source-PR fixture names observed
in the Lane B upstream worktrees. This baseline does not claim those source
fixtures are present in PR #620.

This file records the current PR #620 planning-tree state so the post-release
work delta is measurable against a fixed reference. Bar 2 close criteria are
normative in `.planning/trajectory-5/debate/00-SYNTHESIS.md` Lane B and
`SHIP-BAR-TRACKER.md` Claim B. Source PRs may contain fixture files that are
not present here; this document treats them as upstream evidence to integrate,
not as evidence already landed in #620.
...
Each must close with: enforced production call site, spec MUST citation, and a
production-call-path conformance fixture that fails when wiring is removed.
Pre-release work in the #620 tree: zero of four are present here.
...
| Existing conformance fixture file | NONE IN #620 (verified by `ls crates/chio-conformance/tests/`); upstream cleanup uses `b2_receipt_v2_failclosed_pre_dispatch.rs` |
| Expected post-release work fixture path | `crates/chio-conformance/tests/b2_receipt_v2_failclosed_pre_dispatch.rs` |
...
| Expected post-release work fixture path | Name to be finalized by the source PR; the fixture must be a B4 `crates/chio-conformance/tests/b4_bilateral_dsse_*.rs` file proving full DSSE PAE conformance, not the interim signature-slice regression alone |
| Current enforcement status | PENDING -- source PR evidence is signature-slice only; full §6 DSSE PAE conformance is not closed by #620 |
...
| Conformance fixtures in `crates/chio-conformance/tests/` matching the current expected names | 0 of 4 in #620 | B1/B2/B3 plus full B4 DSSE PAE fixture |
| Upstream evidence markers present | 0 of 4 in #620 | Fixture headers cite spec anchor / Spec MUST / reverts-to-fail or an equivalent evidence marker |
...
3. **Artifact C (production-call-path conformance fixture)**: file exists at
   ...
   production call path; carries a spec/reverts-to-fail evidence marker; FAILS
   when the enforcement is removed (proven by inverting the patch under
   review). A literal `negative-conformance` comment is acceptable but not
   required when the upstream fixture already carries an equivalent header.
...
6. `scripts/check-bounded-ship-bar.sh` Bar-2 block PASSes:
   - B1, B2, and B3 expected files exist under their current upstream names;
   - B4 full DSSE PAE conformance fixture exists, not just the interim
     signature-slice regression;
   - each fixture carries a spec/reverts-to-fail evidence marker;
