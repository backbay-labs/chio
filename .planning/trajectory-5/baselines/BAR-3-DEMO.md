...
This file records the CURRENT (pre-trj5) state of Bar 3 so the post-trj5
...
## examples/ directory state (current, pre-trj5)
...
## mcp-remote bridge (current, pre-trj5)
...
## Bilateral cosign primitives in `chio-federation` (current, pre-trj5)
...
## DSSE PAE signing (current, pre-trj5)
...
## `chio receipt explain` (current, pre-trj5)
...
| Surfaces "policy verdict disagreement" as top-level diagnostic | NOT TODAY (TRJ5-C4.1 adds this per W3 R4-MINOR-9 fix; effort bumped M->L) |
| Renders bilateral chain with parent->child arrows | NOT TODAY (TRJ5-C4.1 adds this) |

## Aggregate baseline state

| Bar 3 sub-criterion | Baseline (pre-trj5) | Target (post-trj5) |
|---|---|---|
...
5. `.github/workflows/chiodome-demo-continuous.yml` (TRJ5-C6.3) is
   green for 7 consecutive nights pre-tag.
6. `tools/diff-stable.py` (or Rust binary) (TRJ5-C6.4) verifies the
