...
The tracker is consumed by `scripts/check-release work-ship-bar.sh` (companion to `scripts/release work-preflight.sh`; both are required for release work closeout). The script:
...
`scripts/release work-preflight.sh` covers kickoff prerequisites (planning artifacts, OWNERS population, releases.toml block, Wave-2/3/4 review trail, drift cleanup); `scripts/check-release work-ship-bar.sh` covers the three closing bars. The pattern matches the trj4 close-bar tracker at `../trajectory-4/closeout/CLOSE-BAR-TRACKER.md`.
...
| **Validator** | Wave-2 reviewer + `scripts/check-release work-ship-bar.sh` Bar-1 block. |
...
| **Trj5 ticket(s)** | release work-A1, release work-A2, release work-A3, release work-A4, release work-A5 (Lean; renumbered from A6 per Wave 3), release work-A7 (and dependents per `lane-a-floor/tickets.md`); each sub-lane closes under its `release work-A<n>.E` Evidence Gate ticket |
...
| **Validator** | Wave-2 reviewer + `scripts/check-release work-ship-bar.sh` Bar-2 block. The script asserts that each conformance test exists, that inverting the patch under review causes the test to fail, and that the production call sites match the corrected line citations. |
...
| **Trj5 ticket(s)** | release work-B0 (architectural prerequisite), release work-B1, release work-B2, release work-B3, release work-B4 (DSSE signing), release work-B1.E / B2.E / B3.E / B4.E (and dependents per `lane-b-wiring/tickets.md`). |
| **Status** | PARTIAL. Four signed negative conformance fixtures are expected from open PR branches: `b1_capability_v2_single_entry_no_bypass` (PR #612 on top of #606 B0 async-trait), `b2_receipt_v2_failclosed_pre_dispatch` (PR #611), `b3_anchor_batch_sync_path_rejected_under_public_witness` (PR #609), and `b4_bilateral_dsse_pae_only_is_conformant` (PR #610). The release bar does not become DONE until those PRs merge, dependent branches rebase, fixtures are regenerated from merged `main`, and checks are green on the integrated merge SHA. |
...
| **Validator** | Wave-2 reviewer + `scripts/check-release work-ship-bar.sh` Bar-3 block. The script asserts the example runs end-to-end on a fresh checkout, the golden file matches, and the release tag is recorded. |
