...
rates), it adopts the trj5 Evidence Gate as its close bar, and it lays out 46
...
**Patch**: TRJ5-A1.2 should be split into two tickets: TRJ5-A1.2a (run the
baseline) and TRJ5-A1.2b (publish the per-crate numbers). After TRJ5-A1.2a,
...
**Patch**: add a TRJ5-A1.0 ticket: "Audit `.cargo/mutants.toml`
...
under `audits/evidence/TRJ5-A1.0/exclude-audit.md` with each exclusion
...
TRJ5-A1.10 reproducibility check ("re-running the workflow on the same
...
**Patch**: TRJ5-A1.1 should verify the current `status_at_capture` of the
last 7 nightly runs and document any flake. If the workflow has been red,
TRJ5-A1.1 owns un-flaking it before the per-crate measurement starts.
...
The "TRJ5-A2.21 absorbs a 21st" hedge is fine.
...
**Patch (BLOCKER)**: each TRJ5-A2.<n> ticket must, before close,
...
### 2.4 Backfillable in trj5 vs blocked on architecture (MAJOR)

Per Risk Register R3: "More than 4 of 21 rows tag as
`provable-only-in-trj6`. At that point the trj5 banner claim is too soft
...
**Patch (MAJOR)**: TRJ5-A2.22 should DELETE the `needs_real_run` clause
...
`play_integrity_token_replay` with `Depends-on: TRJ5-A1.1, TRJ4-033`.
...
**Patch**: TRJ5-A2 should fail closed if `TRJ4-033` is not in its
...
**Patch (MAJOR)**: each harness file in TRJ5-A3.1/A3.2/A3.3 must
...
Add a TRJ5-A3.0 ticket: "Run each proposed Kani invariant locally with
the proposed bounds. Capture wall-clock, memory, and exit status to
`audits/evidence/TRJ5-A3.0/local-bound-validation.md`. If any invariant
exceeds 30 minutes locally, escalate (R-new) before TRJ5-A3.1 starts."
...
**Patch (BLOCKER)**: TRJ5-A3.5 must own the workflow rewrite, with a
specific named approach. Add a sub-ticket TRJ5-A3.5a "Extend
...
The current TRJ5-A3.5 description ("Wire all three harnesses into
...
TRJ5-A3.4 updates `formal/proof-manifest.toml`. I confirmed
...
**Patch (MINOR)**: TRJ5-A3.4 should reference the actual file
...
**Patch (MAJOR)**: TRJ5-A4.2 must include a "feasibility spike" sub-task:
...
a link in `audits/evidence/TRJ5-A4.2/feasibility-spike.md`. If Apalache
0.50.x does not handle the encoding, TRJ5-A4.2 escalates - the only
...
**Patch (MINOR)**: TRJ5-A4.3 must record the apalache run wall-clock
before AND after the bump, captured in
`audits/evidence/TRJ5-A4.3/length-budget.md`. If the post-bump run
...
require `audits/evidence/TRJ5-A4.4/branch-protection-screenshot.png` or
...
`audits/evidence/TRJ5-A4.4/branch-protection.png`" to TRJ5-A4.4
acceptance.
...
checking for this. The cascade-update ticket (TRJ5-A4.5) is the right
...
**Patch (MAJOR)**: TRJ5-A5.3 should state at least three theorems, not
one:
...
TRJ5-A5.1 adds a `lean.yml` workflow. The existing situation: Lean is
...
plan's TRJ5-A5.1 absorbs this work but it is sized M, not S, because
Lean 4 toolchain installation in CI is non-trivial (lake build, package
manager setup, deterministic version pin).

**Patch (MINOR)**: TRJ5-A5.1 should be re-scoped from M to L given the
...
### 6.1 TRJ5-A1.10 (banner update) - PASS
...
### 6.2 TRJ5-A2.10 (`native_channel_replay` rewrite) - INCOMPLETE
...
**Fails Artifact A and D specificity.** Patch: each TRJ5-A2.<n> ticket
needs an Artifact A line and an Artifact D revert-and-rerun procedure.

### 6.3 TRJ5-A3.1 (chio-attest-verify Kani harness) - BLOCKED
...
### 6.4 TRJ5-A3.2 (chio-anchor Kani harness) - BLOCKED

Same as 6.3, plus the Kani-budget feasibility spike is missing
(Section 3.2). **Blocked.**

### 6.5 TRJ5-A4.1 (Allow split + ReceiptBeforeAllow) - PASS-with-watchout
...
**Patch (MINOR)**: add to TRJ5-A4.1 acceptance: "remove the
...
### 6.6 TRJ5-A4.4 (apalache-temporal required) - PASS-with-watchout
...
### 6.7 TRJ5-A5.3 (negotiation_safety re-prove) - INCOMPLETE
...
TRJ5-A2.<n> tickets each cite the trio "caught >= 1", "needs_real_run:
...
TRJ5-A5.3 says "The proof body is **not** `rfl` against
...
**Patch (MINOR)**: TRJ5-A5.3 acceptance should additionally state: "the
...
exclusions are broad globs. The plan does not re-audit these for trj5
...
**Patch**: tied to Section 1.2 patch (TRJ5-A1.0 audit ticket).
...
`single_owner = "@bb-connor"` for the entire trajectory; `human_assignment
...
Every TRJ5-A<n>.<m> ticket maps to one of the five owner classes via the
...
The Lane A Kani harness (TRJ5-A3.2) targets functions in
...
**Patch (MINOR)**: TRJ5-A3.2 should explicitly call out Lane B
...
in trj5, defer to trj6, or downgrade banner. The plan accepts all three;
...
reviews each of the 21 rows and tags `provable-in-trj5` /
...
in {`provable-in-trj5`, `provable-only-in-trj6`, `architecture-blocked`}.
...
- `apalache-temporal.yml` (tla-rewrites.md:19, A4 - currently advisory,
  promoted by TRJ5-A4.4).
- `lean.yml` (lean4-fix.md:166, A5 - to be created by TRJ5-A5.1).
...
- `ci.yml` - the PR-tier mainline workflow. The Kani PR job
  ...
  PR queue.
- `close-bar-tracker.yml` - the meta-workflow that runs the trj5
  ...
  workflow. If `check-trj5-evidence-gate.sh` is a Wave 1 deliverable
...
that owns the change. Add to the Evidence Gate close bar: each TRJ5-A
...
specified (TRJ5-A4.4). The plan does not specify whether the new
...
**Patch (MINOR)**: add a TRJ5-A3.6 ticket: "Promote the new chio-attest-
...
- Q2: Does Apalache 0.50.x support the bounded transitive-closure
  unrolling encoding proposed in TRJ5-A4.2? Without a feasibility
...
- Q4: Are there `formal/proof-manifest.toml` and
  ...
  one? TRJ5-A3.4 references the former; the actual file I located is
  the latter.
- Q5: Will the Lean CI lane be blocking on PR (required) or only
  advisory at TRJ5-A5 close? The plan does not say. If advisory, a
...
- `/.../lane-a-floor/lean4-fix.md`: rewrite lines 75-85 to match the
  ...
  `VerifiedCapability`. Expand TRJ5-A5.3 to require three theorems, not
  one (Section 5.1, 5.2). Re-scope TRJ5-A5.1 from M to L.
...
- `/.../lane-a-floor/tickets.md`: each TRJ5-A2.<n> ticket gains a
  required Artifact A line (Section 6.2). TRJ5-A2.22 changes scope
  from doc-update to script-deletion (Section 2.6). Add TRJ5-A1.0
  exclusion-audit and TRJ5-A3.0 Kani-feasibility-spike tickets.
...
- `/.../lane-a-floor/tla-rewrites.md`: add feasibility-spike sub-task
  to TRJ5-A4.2 (Section 4.2). Add wall-clock evidence requirement to
  TRJ5-A4.3 (Section 4.3). Add branch-protection screenshot evidence
  to TRJ5-A4.4 (Section 4.4).
