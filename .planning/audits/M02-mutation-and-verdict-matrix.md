...
| `M02.P5.T2` | `merged` | Satisfied locally | `drivers/typescript/run_scenarios.ts` exposes the node-http transport-client driver (`TYPESCRIPT_NODE_HTTP_DRIVER`), and `sdks/typescript/packages/conformance/test/verdict_matrix.test.ts` exercises it via `vitest`. The driver reports all 48 scenarios as `unsupported` without a live sidecar (matching the manifest's `transport-client` registration); the test asserts the unsupported diagnostic and the tuple-from-evaluate-response projection. Gate: `cd sdks/typescript/packages/conformance && npm test -- verdict_matrix` runs three vitest cases green. |
| `M02.P5.T3` | `pending` | Satisfied locally | `drivers/wasm-browser/run.sh` invokes `wasm-pack test --headless --chrome` against `crates/chio-kernel-browser` and chains `cargo test -p chio-kernel-browser --test verdict_matrix_wasm` for the native lane. The integration test under `crates/chio-kernel-browser/tests/verdict_matrix_wasm.rs` loads all 48 scenarios via `include_str!`, runs the 12 capability-subset scenarios through `evaluate_pure`, and reports the remaining 36 as unsupported (no revocation store, execution nonce store, or guard pipeline in the browser surface). Gate: `cargo test -p chio-kernel-browser --test verdict_matrix_wasm --quiet` runs one test green. |
...
| `M02.P5.T5` | `pending` | Satisfied locally | `crates/chio-conformance/verdict_matrix/src/cross_language.rs` defines `CrossLanguageReport`, `CrossLanguageDivergence`, and `diff_cross_language(manifest, expected, report)` with fail-closed semantics: required drivers MUST emit a tuple per scenario; non-required drivers MAY skip scenarios they cannot evaluate; whenever any driver does emit a tuple it must agree with the expected value AND every other driver that emits a tuple. The integration test at `crates/chio-conformance/verdict_matrix/tests/verdict_matrix_cross_language.rs` runs the rust-kernel driver and the wasm-browser kernel (via the native non-wasm32 path) against the full corpus and asserts zero divergences. The workflow's `required: true` flag is in place, so the gate is wired to fail PRs on divergence. Gate: `cargo test -p chio-conformance --test verdict_matrix_cross_language --quiet` runs four integration tests, the five unit tests in `cross_language::tests`, and the three pre-existing `verdict_matrix::tests` lib units green (12 total). |
| `M02.P5.T6` | `pending` | Satisfied locally | The corpus hash is pinned in `manifest.toml` as `corpus_sha256 = "47e8d5394c807196d9567d97515e786cb1abfb0c7676e54db269ca82c735422f"` (and as `scenario_index_hash` with the matching `sha256:` prefix). `docs/conformance/verdict-matrix.md` contains the "Corpus rotation" runbook. The diff-oracle self-test (`tests/diff_oracle_self_test.rs::manifest_hash_pins_current_scenario_index`) recomputes the index hash and asserts equality with the manifest, so any unauthorised corpus drift fails CI. The dependency on T5 is now satisfied. Gate: `grep -qE 'corpus_sha256 *= *"[0-9a-f]{64}"' crates/chio-conformance/verdict_matrix/manifest.toml && grep -q 'corpus rotation' docs/conformance/verdict-matrix.md`. |
