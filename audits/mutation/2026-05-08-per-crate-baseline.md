...
- **Test scope (workspace-wide; matches CI)**: although the invocation
  ...
  `pending full-sweep measurement` strings
...
- A "target met" claim requires either a full per-crate run OR a
  ...
  This branch currently commits `chio-credentials` cargo-mutants evidence; rows
...
The aggregate truth for the per-crate baseline must have one owner.
Parallel per-crate evidence branches can each carry their own artifacts
under `audits/evidence/mutants/<crate>/`, but aggregate-file edits are
functionally redundant until the integrated tree is regenerated.

1. Merge per-crate evidence branches in any order. The per-crate evidence
   directories do not collide.
2. After all per-crate evidence lands, run `bash audits/mutation/aggregate.sh`
...
3. Update the planning-owned mutation baseline from the same regenerated
   table.
4. Do NOT re-edit the aggregate to "fix" a row from a per-crate branch;
...
If two worker branches landed before this consolidation step, the
...
The chio-kernel-core row is currently BASELINE-GAP until its dedicated
kernel-core evidence lands. After that evidence is available, the
...
`pending full-sweep measurement` strings to
