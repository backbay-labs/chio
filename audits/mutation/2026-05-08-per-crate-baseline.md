...
Until the autogenerated table is regenerated and committed, the
in-progress / queued / BASELINE-GAP markers above are the *current
observed state*, not placeholders for future numbers.

## R2 ownership notice (T5-R2-P0-008)
...
2. After all worker PRs land, run `bash audits/mutation/aggregate.sh`
   ...
   per-crate `<date>.json` summary (T5-R2-P1-005).
...
4. Do NOT re-edit the aggregate to "fix" a row from a worker PR;
   ...
   / `command` / `notes` whitelist (T5-R2-P0-010) preserves operator
...
(73.6% PARTIAL on 62/343 surface; target_met=false per T5-R2-P0-009).
