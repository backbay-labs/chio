...
//!
//! Protocol: `chaos_victim <db_path> <ack_path> <max_receipts>`. Opens the
//! receipt store, then loops: append one receipt, `flush_receipt_writes()` as
//! the durability barrier, and only after a successful flush append an
//! `ack <seq>` line to the ack file (opened O_APPEND, `sync_data` after each
//! line). The ack file is the ground truth of which receipts the store promised
//! were durable; the DB flush happens-before the ack write, so a recovered ack
//! implies a durably-committed receipt.
//!
