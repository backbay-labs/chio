...
weaker than serializable schedules~\cite{TODO_bea_bib}. A saga is a
...
literature; the construction here inherits the saga vocabulary without
inheriting the broader workflow-net soundness obligations, which would
...
machine boundary; the construction here lifts it to the cross-
...
The parent paper supplies four primitives this construction depends on.
The treaty intersection theorem \thm{treaty_admission_iff_predicate_intersection}
gives the bilateral reduction. The ladder-floor stability theorem
...
amendment refinement bridge \thm{amendment_admissible_iff_backward_refinement}
gives the type-level invariant on enactment. The essential-predicate
chain theorem \thm{essential_preserved_chain} gives the trajectory-
invariant composition over amendment sequences. The composition theorem
this construction adds is the response-side analog of the fourth
primitive: TTL-bounded amendments compose with the chain machinery the
parent paper already established.
...
artifact cannot be retroactively denied. The construction does not solve
scheming; it makes the consequence of an agent's destructive action
recordable in the same canonical form the parent paper's receipts
already take.
