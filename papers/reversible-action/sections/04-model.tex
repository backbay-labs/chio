...
\draw[decorate, decoration={brace, mirror, amplitude=4pt}]
  ...
  {per-step $\mathsf{BackwardRefines}$ witness preserves baseline\\[1pt]
   (Theorem~3: baseline preserved at every $t$)};
\end{tikzpicture}
...
(Theorem~3) states that the baseline is preserved at every
instant.}
...
chain of TTL-bounded amendments, each carrying two witnesses (one
that the pre-amendment constitution backward-refines a fixed baseline,
and one per-step witness that the post-amendment constitution
backward-refines the pre-amendment constitution), preserves the
baseline at every instant:
\[
  \forall t,\, \forall A \in \mathit{chain},\
    \mathsf{BackwardRefines}\bigl(\mathsf{activeAt}(A, t),\, \mathit{baseline}\bigr).
\]
The conclusion is pointwise over $A \in \mathit{chain}$ under a
universal quantifier, not inductive over an evolving state. For each
amendment $A$, the proof case-splits on the $\mathsf{activeAt}$
conditional. Outside the window,
$\mathsf{activeAt}(A, t) = \delta.\mathit{old}$, and the conclusion is
the post-expiry hypothesis
$\mathsf{BackwardRefines}(\delta.\mathit{old}, \mathit{baseline})$
applied directly. Inside the window,
$\mathsf{activeAt}(A, t) = \delta.\mathit{new}$, and the conclusion
composes the per-step witness
$\mathsf{BackwardRefines}(\delta.\mathit{new}, \delta.\mathit{old})$
with the post-expiry hypothesis through transitivity of
$\mathsf{BackwardRefines}$. Both arms consume the per-step witnesses
as data rather than discharging by reflexivity, so each amendment in
the chain contributes substantive content.
...
syntactic constitution given the syntactic refinement witness over
$\mathsf{admits}$:
\[
  \mathsf{admits}(c_{\text{new}}, \mathit{rb}) = \text{true}
    \;\wedge\; \bigl(\forall \mathit{rid},\,
       \mathsf{admits}(c_{\text{new}}, \mathit{rid}) = \text{true}
       \Rightarrow \mathsf{admits}(c_{\text{old}}, \mathit{rid}) = \text{true}\bigr)
...
The two theorems range over different objects. The composition theorem
ranges over $\mathsf{Constitution}$ via $\mathsf{BackwardRefines}$
(opaque receipt-to-Boolean closures); the rollback reduction ranges
over $\mathsf{SyntacticConstitution}$ via $\mathsf{admits}$ (decidable
denotation of a predicate ADT). The bridge between the two layers is
the closure-to-syntactic soundness theorem in the parent paper's
$\mathsf{PredicateLang}$: a syntactic constitution induces a closure
constitution whose receipt-by-receipt admission decision agrees with
$\mathsf{admits}$. Once that bridge is invoked, the composition
theorem's $\mathsf{BackwardRefines}$ refines, on each receipt id, to
the syntactic implication
$\mathsf{admits}(c_{\text{new}}, \mathit{rid}) = \text{true} \Rightarrow
 \mathsf{admits}(c_{\text{old}}, \mathit{rid}) = \text{true}$, which is
the hypothesis the rollback reduction consumes. The bridge itself is
not reproved here; the substrate inherits it from $\mathsf{PredicateLang}$.
