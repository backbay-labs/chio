...
\begin{tikzpicture}[
  font=\footnotesize,
  stage/.style={draw, thick, rounded corners=2pt, fill=gray!8, align=center, inner sep=3pt, text width=1.4cm, minimum height=0.85cm},
  gate/.style={draw, thick, diamond, aspect=1.7, fill=blue!7, draw=blue!55!black, align=center, inner sep=0pt, minimum width=1.5cm, minimum height=0.5cm, font=\scriptsize},
  accept/.style={draw, thick, rounded corners=2pt, fill=green!10, draw=green!55!black, align=center, inner sep=3pt, text width=1.5cm, minimum height=0.85cm},
  reject/.style={draw, thick, rounded corners=2pt, fill=red!7, draw=red!60!black, align=center, inner sep=2pt, text width=1.7cm, font=\scriptsize},
  pass/.style={-{Latex[length=1.8mm]}, thick},
  fail/.style={-{Latex[length=1.8mm]}, thick, red!55!black, dashed},
  passlabel/.style={font=\scriptsize, above, inner sep=1pt},
  faillabel/.style={font=\scriptsize, red!55!black, right, inner sep=1pt}
]

\node[stage] (env) at (0, 0) {Envelope\\emitted};
\node[gate, right=0.3cm of env] (g1) {(1)\\capability};
\node[gate, right=0.3cm of g1] (g2) {(2)\\class};
\node[gate, right=0.3cm of g2] (g3) {(3)\\TTL $> 0$};
\node[gate, right=0.3cm of g3] (g4) {(4)\\rollback\\slot};
\node[accept, right=0.3cm of g4] (exec) {Executed\\+ receipt};
...
\node[reject, below=0.95cm of g1] (r1) {\texttt{cap-denied}};
\node[reject, below=0.95cm of g2] (r2) {\texttt{class-}\\\texttt{mismatch}};
\node[reject, below=0.95cm of g3] (r3) {\texttt{ttl-invalid}};
\node[reject, below=0.95cm of g4] (r4) {\texttt{rollback-}\\\texttt{missing}};
...
\node[draw=red!55!black, dashed, thick, rounded corners=2pt, fit=(r1) (r2) (r3) (r4), inner sep=3pt, label={[font=\scriptsize\itshape, red!55!black]below:rejection receipt with named code}] {};
...
The grammar carries three structural commitments whose proofs are discharged in companion work~\cite{programmableSovereignty2026}.

\emph{Theorem 1 (Bounded executive action carries TTL and rollback slot).} A reversible or destructive call's envelope is well-typed only if it carries a positive TTL witness and a rollback receipt slot. The claim is a definitional bridge documenting the type-level invariant; its substantive use is the type-checker's refusal to construct envelopes without the discipline.

\emph{Theorem 2 (Rollback admission composes with refinement).} When the admission predicate is amended (the policy changes), a rollback receipt that closes a previously-admitted call remains admissible under the amended predicate, provided the amendment is backward-refining. Rollback obligations do not vanish across policy changes.

\emph{Theorem 3 (Treaty admission iff predicate intersection).} A destructive call admits only if both the operator polity and the host polity admit it under their respective predicates. This is the structural reason a single-party operator manipulation does not suffice to admit a destructive tool call against a counterparty whose policy forbids it.
