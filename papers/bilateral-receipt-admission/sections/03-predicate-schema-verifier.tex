...
\begin{tikzpicture}[
  font=\footnotesize,
  envelope/.style={draw, thick, rounded corners=3pt, inner sep=8pt},
  field/.style={draw, rectangle, minimum height=4.2mm, text width=38mm, align=left, font=\scriptsize, inner sep=2pt, fill=blue!4},
  sigbox/.style={draw, rectangle, minimum height=7mm, minimum width=26mm, align=center, font=\scriptsize, inner sep=3pt, fill=green!8},
...
\node[envelope, label={[font=\bfseries\small]above:DSSE Envelope (payload + 2 signatures)}] (env) {%
  \begin{tikzpicture}
    \node[hdrlabel] (payhead) {payload (canonical JCS bytes)};
    ...
    \node[hdrlabel, right=18mm of payhead] (sighead) {signatures};
    \node[sigbox, below=2mm of sighead.south, anchor=north] (s1) {sig$_1$ by $k_1$};
    \node[sigbox, below=1.2mm of s1.south, anchor=north] (s2) {sig$_2$ by $k_2$};
    \draw[decorate, decoration={brace, amplitude=4pt, mirror}]
      ([xshift=1mm]f1.north east) -- ([xshift=1mm]f9.south east)
      node[midway, right=6pt, font=\scriptsize, align=left] (brace) {bindingTuple\\(ten fields)};
  \end{tikzpicture}
};
\node[below=5mm of env, font=\small] (hash) {$\mathit{subjectDigest} = H(\mathit{canonical}(\mathit{bindingTuple}))$};
\draw[->, thick] (env.south) -- (hash.north);
\end{tikzpicture}
...
\begin{tikzpicture}[
  ...
  gate/.style={draw, rectangle, rounded corners=2pt, minimum height=6mm, minimum width=46mm, align=left, inner sep=3pt},
  ...
  accept/.style={draw, rectangle, double, minimum height=6mm, minimum width=46mm, align=center, fill=green!10},
