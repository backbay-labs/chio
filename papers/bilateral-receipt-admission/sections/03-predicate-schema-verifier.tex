...
\node[envelope, label={[font=\bfseries\small]above:DSSE Envelope (payload + 2 signatures)}] (env) {%
  \begin{tikzpicture}
    ...
    \draw[decorate, decoration={brace, amplitude=7pt, mirror, raise=2pt}]
      (f1.north east) -- (f9.south east);
    \node[font=\scriptsize, align=left, right=12pt] at ($(f1.north east)!0.5!(f9.south east)$) (brace) {\textbf{bindingTuple}\\(ten fields)};
