...
9. **chiodos-3vendor build.** No crate named `chio-chiodos-3vendor` exists on `main` OR on `codex/chiodos-7-8-live-treaty-buyer-closure` -- the actual crate is `chiodos-three-vendor-example` (under `examples/chiodos-3vendor/`). On the chiodos worktree (`/Users/connor/.codex/worktrees/985a/arc`): `cargo check -p chiodos-three-vendor-example` exits 0; `cargo test -p chio-chiodos-runtime --tests` exits 0 across all 8 test binaries (admission, buyer_review, kernel_hook, ops, orchestration, pheromone_policy, store, treaty, trust); all suites pass. CLAIMS HOLD modulo the crate-name mismatch in the iter-8 charter itself.
